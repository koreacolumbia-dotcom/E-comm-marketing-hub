...
def process_data(posts: List[Post]):
    ...
    for b in BRAND_LIST:
        ...
        for item in brand_map[b]:
            ...
            normalized_text = normalize_text(item.get("text", "")).lower()
            if platform == "dcinside":
                key = (platform, normalized_text)
            else:
                key = (
                    platform,
                    item.get("url", ""),
...
def _summary_table_html(summary_df: pd.DataFrame, platform: str) -> str:
    ...
    ranked_df = summary_df.reset_index(drop=True).copy()
    total_mentions = int(ranked_df["total_mentions"].sum()) if "total_mentions" in ranked_df else 0
    top_df = ranked_df.head(5)
    rest_df = ranked_df.iloc[5:]
    columbia_block = ""

    columbia_rows = ranked_df[ranked_df["brand"] == "컬럼비아"]
    if not columbia_rows.empty:
        columbia_index = int(columbia_rows.index[0]) + 1
        columbia_row = columbia_rows.iloc[0]
        columbia_share = (int(columbia_row["total_mentions"]) / total_mentions * 100) if total_mentions else 0
        columbia_block = f'''
        <div class="mt-4 rounded-3xl border border-red-200 bg-gradient-to-r from-red-50 via-white to-red-50 p-4 shadow-sm">
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="text-[11px] font-black tracking-[0.18em] text-red-500">COLUMBIA WATCH</div>
              <div class="mt-2 flex items-center gap-3">
                <span class="inline-flex h-9 min-w-9 items-center justify-center rounded-full bg-red-500 px-3 text-sm font-black text-white">#{columbia_index}</span>
                <span class="text-lg font-black text-slate-900">컬럼비아 현재 순위</span>
              </div>
            </div>
            <div class="flex flex-wrap gap-2 text-xs font-black">
              <span class="rounded-full bg-white px-3 py-2 text-slate-600">Posts {int(columbia_row["posts_count"]):,}</span>
              <span class="rounded-full bg-white px-3 py-2 text-slate-600">Mentions {int(columbia_row["total_mentions"]):,}</span>
              <span class="rounded-full bg-red-500 px-3 py-2 text-white">Share {columbia_share:.1f}%</span>
            </div>
          </div>
        </div>
        '''
...
def _brand_sections_html(brand_map: Dict[str, List[dict]], platform: str) -> str:
    ...
    for section_idx, (brand, _) in enumerate(_platform_brand_counts(brand_map, platform, prioritize_columbia=True)):
        ...
        for item in raw_items:
            ...
            text_key = _ui_compact_text(item.get("text", "")).lower()
            if platform_key == "dcinside":
                card_key = (platform_key, text_key)
            else:
                card_key = (
                    platform_key,
                    item.get("url", ""),
...
<head>
  ...
  <style>
    ...
    .filter-chip.active span {{ color:rgba(255,255,255,.82) !important; }}
    .filter-chip.active, .filter-mode-btn.active, .brand-filter-btn.active, .cafe-filter-btn.active {{
      color:#fff !important;
      border-color:#17305f !important;
      background:linear-gradient(135deg, #1e3a6d, #0f172a) !important;
      box-shadow:0 16px 28px rgba(15,23,42,.20) !important;
    }}
...
def _brand_sections_html(brand_map: Dict[str, List[dict]], platform: str) -> str:
    sections = []
    for brand, _ in _platform_brand_counts(brand_map, platform, prioritize_columbia=True):
        seen_items = set()
        items = []
        for item in [x for x in brand_map.get(brand, []) if x.get("platform") == platform]:
            key = (
                item.get("platform", ""),
                item.get("url", ""),
                normalize_text(item.get("text", "")).lower(),
