...
function renderPurchasePattern(section){
  ...
  const cards = `<div class="grid-4">
    <div class="card"><div class="kicker">Multi-product Buyers</div><div class="kpi">${num(summary.multi_item_buyers || 0)}</div><div class="kpi-sub">여러 제품 구매 고객</div></div>
    <div class="card"><div class="kicker">Multi-product %</div><div class="kpi">${pct(summary.multi_item_pct || 0)}</div><div class="kpi-sub">전체 구매자 대비 비중</div></div>
    <div class="card"><div class="kicker">Multi-color Buyers</div><div class="kpi">${num(summary.multi_color_buyers || 0)}</div><div class="kpi-sub">동일 아이템 컬러별 구매</div></div>
    <div class="card"><div class="kicker">Multi-size Buyers</div><div class="kpi">${num(summary.multi_size_buyers || 0)}</div><div class="kpi-sub">동일 아이템 사이즈별 구매</div></div>
    <div class="card"><div class="kicker">Repeat-item Buyers</div><div class="kpi">${num(summary.repeat_item_buyers || 0)}</div><div class="kpi-sub">동일 아이템 반복 구매</div></div>
  </div>`;
  const multiItem = table(section?.multi_item_rows||[], [['member_id','Member ID'],['distinct_product_count','Distinct Products'],['max_distinct_products_in_order','Max In Order'],['avg_distinct_products_per_order','Avg In Order'],['multi_product_order_count','Multi-product Orders'],['top_product','Top Product']], ['distinct_product_count','max_distinct_products_in_order','avg_distinct_products_per_order','multi_product_order_count'], 'pattern-multi-item');
  const multiColor = table(section?.multi_color_rows||[], [['member_id','Member ID'],['product_name','Product Name'],['multi_color_product_count','Product Count'],['max_color_count','Max Colors'],['top_product','Top Product']], ['multi_color_product_count','max_color_count'], 'pattern-multi-color');
  const multiSize = table(section?.multi_size_rows||[], [['member_id','Member ID'],['product_name','Product Name'],['multi_size_product_count','Product Count'],['max_size_count','Max Sizes'],['top_product','Top Product']], ['multi_size_product_count','max_size_count'], 'pattern-multi-size');
  const repeatItem = table(section?.repeat_item_rows||[], [['member_id','Member ID'],['product_name','Product Name'],['repeat_item_product_count','Repeat Products'],['max_repeat_item_orders','Max Orders'],['top_product','Top Product']], ['repeat_item_product_count','max_repeat_item_orders'], 'pattern-repeat-item');
  return `<section>
    <div class="section-head"><div><div class="section-title">PURCHASE PATTERN</div><h2>고객 구매 패턴 분석</h2></div></div>
    ${cards}
    <div class="grid-2" style="margin-top:18px">
      <div class="card"><div class="section-title">여러 제품 구매 고객</div>${multiItem}</div>
      <div class="card"><div class="section-title">동일 아이템 컬러별 구매 고객</div>${multiColor}</div>
      <div class="card"><div class="section-title">동일 아이템 사이즈별 구매 고객</div>${multiSize}</div>
      <div class="card"><div class="section-title">동일 아이템 반복 구매 고객</div>${repeatItem}</div>
    </div>
  </section>`;
}
