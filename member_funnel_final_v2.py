...
:root{--bg:#eef4fb;--shell:rgba(255,255,255,.64);--shell-line:rgba(255,255,255,.72);--card:#ffffff;--line:#dbe5f3;--line-2:#cdd9ec;--text:#0f172a;--muted:#64748b;--navy:#17305f;--navy-2:#0f172a;--blue:#2563eb;--blue-soft:#93c5fd;--shadow-lg:0 24px 70px rgba(15,23,42,.10);--shadow-md:0 16px 34px rgba(15,23,42,.08);--shadow-sm:0 10px 24px rgba(15,23,42,.05)}
*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;color:var(--text);font-family:'Pretendard','Noto Sans KR','Apple SD Gothic Neo','Malgun Gothic',Arial,sans-serif;background:radial-gradient(circle at 12% 18%, rgba(59,130,246,.20), transparent 28%),radial-gradient(circle at 84% 14%, rgba(34,197,94,.10), transparent 24%),radial-gradient(circle at 72% 74%, rgba(249,115,22,.08), transparent 24%),linear-gradient(180deg,#f8fbff 0%,#eef4fb 52%,#e8eff8 100%);background-size:120% 120%;animation:ambientShift 18s ease-in-out infinite alternate}
.page{width:min(1600px,calc(100vw - 24px));margin:12px auto;padding:16px 16px 40px;border:1px solid var(--shell-line);border-radius:36px;background:var(--shell);box-shadow:var(--shadow-lg);backdrop-filter:blur(24px)}
.hero{position:relative;overflow:hidden;border-radius:32px;padding:28px 28px 24px;background:#020617;color:#fff;box-shadow:0 20px 50px rgba(15,23,42,.20)}
.hero:before{content:'';position:absolute;inset:-120px auto auto -80px;width:260px;height:260px;border-radius:999px;background:radial-gradient(circle,rgba(59,130,246,.24),transparent 70%);filter:blur(10px);animation:floatBlobReverse 14s ease-in-out infinite}
.hero:after{content:'';position:absolute;right:-52px;top:-42px;width:260px;height:260px;border-radius:999px;background:radial-gradient(circle,rgba(59,130,246,.30),transparent 70%);filter:blur(10px);animation:floatBlob 12s ease-in-out infinite}
.hero-grid{position:relative;z-index:1;display:grid;grid-template-columns:minmax(0,1.45fr) minmax(460px,.95fr);gap:26px;align-items:stretch}
.eyebrow{display:inline-flex;align-items:center;padding:8px 14px;border-radius:999px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.10);font-size:11px;font-weight:900;letter-spacing:.18em;text-transform:uppercase}
h1{margin:18px 0 10px;font-size:60px;line-height:1;letter-spacing:-.05em}
.hero p{max-width:900px;margin:0 0 20px;font-size:15px;font-weight:700;line-height:1.75;color:rgba(255,255,255,.82)}
.hero-meta{display:flex;gap:10px;flex-wrap:wrap}.period-chip{display:inline-flex;align-items:center;justify-content:center;min-width:78px;height:44px;padding:0 18px;border-radius:999px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.08);color:#fff;text-decoration:none;font-size:13px;font-weight:900;line-height:1;transition:all .18s ease;backdrop-filter:blur(6px)}.period-chip:hover{transform:translateY(-1px);box-shadow:0 10px 18px rgba(15,23,42,.18)}.period-chip.active{color:#0f172a;background:#fff;border-color:#fff;box-shadow:0 16px 28px rgba(255,255,255,.14)}
.hero-kpis{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.hero-stat{border:1px solid rgba(255,255,255,.10);background:rgba(255,255,255,.10);padding:18px 20px;border-radius:24px;backdrop-filter:blur(10px);box-shadow:inset 0 1px 0 rgba(255,255,255,.04)}.hero-stat .label{color:rgba(255,255,255,.58)}.hero-stat .value{font-family:'Space Grotesk','Pretendard',sans-serif;font-size:38px;font-weight:700;line-height:1.05;letter-spacing:-.05em;color:#fff}
.toolbar{display:flex;justify-content:space-between;align-items:center;gap:18px;margin:22px 0 18px}.tabs,.subtabs{display:flex;gap:10px;flex-wrap:wrap}
.tab-btn,.subtab-btn,.table-expand-btn,.download-btn{border:1px solid rgba(148,163,184,.35);background:rgba(255,255,255,.95);padding:0 18px;height:44px;border-radius:999px;font-size:12px;font-weight:900;color:#334155;line-height:1;transition:all .18s ease;box-shadow:0 6px 14px rgba(15,23,42,.04);cursor:pointer;text-decoration:none;display:inline-flex;align-items:center}
.tab-btn:hover,.subtab-btn:hover,.table-expand-btn:hover,.download-btn:hover{transform:translateY(-1px);box-shadow:0 10px 18px rgba(15,23,42,.08)}
.tab-btn.active,.subtab-btn.active{color:#fff;background:linear-gradient(135deg,#1e3a6d,#0f172a);border-color:#17305f;box-shadow:0 16px 28px rgba(15,23,42,.20)}
.summary-card,.card{background:rgba(255,255,255,.88);border:1px solid rgba(255,255,255,.70);border-radius:28px;box-shadow:var(--shadow-sm);backdrop-filter:blur(12px)}
.summary-card{padding:24px 26px;margin-bottom:24px;background:linear-gradient(135deg,rgba(37,99,235,.10),rgba(255,255,255,.94) 34%,rgba(37,99,235,.06))}.summary-card ul{margin:10px 0 0 16px;padding:0}.summary-card li{margin:10px 0;font-size:15px;font-weight:800;line-height:1.6}
.panel{display:none;opacity:0;transform:translateY(18px);pointer-events:none}.panel.active{display:block;opacity:1;transform:translateY(0);pointer-events:auto;animation:panelIn .65s cubic-bezier(.2,.7,.2,1) both}
.section-head{display:flex;justify-content:space-between;align-items:flex-end;gap:16px;margin:30px 0 16px}.section-title{font-size:12px;font-weight:900;letter-spacing:.18em;text-transform:uppercase;color:#64748b}.section-head h2{margin:6px 0 0;font-size:24px;line-height:1.2;letter-spacing:-.03em}
.download-row{display:flex;gap:12px;flex-wrap:wrap}.download-btn{background:linear-gradient(135deg,#1e3a6d,#0f172a);color:#fff;border-color:#17305f;box-shadow:0 16px 28px rgba(15,23,42,.18)}
...
.card{padding:22px;min-height:136px}[data-animate],.card,.channel-panel{opacity:0;transform:translateY(18px) scale(.985)}.panel.active [data-animate],.panel.active .card,.panel.active .channel-panel{animation:revealUp .68s cubic-bezier(.2,.7,.2,1) forwards}
.kicker,.label{font-size:11px;font-weight:900;letter-spacing:.16em;text-transform:uppercase;color:var(--muted)}.kpi-sub,.stack-meta{margin-top:8px;font-size:12px;font-weight:800;color:var(--muted)}.value,.kpi{margin-top:10px;font-size:30px;font-weight:1000;line-height:1.06;word-break:break-word;letter-spacing:-.04em}
.chart-row{display:grid;grid-template-columns:92px 1fr 60px;gap:10px;align-items:center;margin:12px 0}.chart-label,.chart-metric{font-size:12px;font-weight:900}.chart-track{height:10px;background:#e6edf7;border-radius:999px;overflow:hidden}.chart-fill{display:block;height:100%;background:linear-gradient(90deg,#2b5cff,#84b6ff);border-radius:999px;transform-origin:left center;transform:scaleX(0)}.panel.active .chart-fill{animation:growBar .9s cubic-bezier(.2,.7,.2,1) forwards;animation-delay:calc(var(--index,0) * 55ms)}
.donut-wrap{display:grid;grid-template-columns:120px 1fr;gap:14px;align-items:center}.donut{width:120px;height:120px;border-radius:50%;box-shadow:inset 0 0 0 10px rgba(255,255,255,.62),0 12px 24px rgba(37,99,235,.12)}.legend-item{display:flex;justify-content:space-between;gap:10px;align-items:center;margin:8px 0;font-size:12px;font-weight:900}.legend-dot{width:10px;height:10px;border-radius:50%;background:#2451e6;display:inline-block;margin-right:8px;box-shadow:0 0 0 4px rgba(36,81,230,.10)}.legend-dot.alt{background:#93c5fd;box-shadow:0 0 0 4px rgba(147,197,253,.18)}
.product-card{display:flex;gap:14px;align-items:center}.thumb{width:48px;height:48px;border-radius:14px;object-fit:cover;border:1px solid var(--line);background:#fff}.thumb-empty{display:flex;align-items:center;justify-content:center;background:#eef3ff;color:#64748b;font-size:11px;font-weight:900}.mini-title{margin-top:4px;font-size:15px;font-weight:900;line-height:1.35}
.table-meta,.table-tools{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;font-size:12px;font-weight:800;color:var(--muted)}.table-wrap{overflow:auto;border:1px solid rgba(219,229,243,.95);border-radius:24px;background:rgba(255,255,255,.96);box-shadow:0 10px 24px rgba(15,23,42,.04)}.data-table{width:100%;min-width:860px;border-collapse:collapse}th,td{padding:13px 12px;border-bottom:1px solid #eef3f8;text-align:left;font-size:12px;font-weight:800;white-space:nowrap}th{background:#f8fbff;color:#64748b;text-transform:uppercase;letter-spacing:.08em;font-size:11px;position:sticky;top:0}td.num,th.num{text-align:right}.is-hidden{display:none}
...
@media (max-width:1280px){.hero-grid{grid-template-columns:1fr}.hero-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media (max-width:1100px){.grid-4,.grid-3,.grid-2{grid-template-columns:1fr}.page{width:min(100vw - 16px,1600px);padding:12px 12px 28px}.hero{padding:24px 20px}.hero-kpis{grid-template-columns:1fr}.toolbar{flex-direction:column;align-items:flex-start}h1{font-size:42px}}
@media (prefers-reduced-motion: reduce){*,*::before,*::after{animation:none!important;transition:none!important}.panel{opacity:1;transform:none;pointer-events:auto}[data-animate],.card,.channel-panel{opacity:1;transform:none}.chart-fill{transform:none}}
</style>
</head>
<body>
<div class="page">
<section class="hero"><div class="hero-grid"><div><div class="eyebrow">External Signal Premium Style · CRM Funnel</div><h1>Member Funnel</h1><p>채널별 행동 데이터와 CRM 액션 대상을 한 화면에서 빠르게 읽고, 바로 추출할 수 있게 재정렬했습니다. USER VIEW는 USER_ID 단위 대표행 기준 액션 분석, TOTAL VIEW는 기존 회원 전체 분석입니다.</p><div class="hero-meta">__PERIOD_NAV__</div></div><div class="hero-kpis"><div class="hero-stat"><div class="label">Sessions</div><div class="value">__SESSIONS__</div></div><div class="hero-stat"><div class="label">__COUNT_LABEL__</div><div class="value">__BUYERS__</div></div><div class="hero-stat"><div class="label">__REVENUE_LABEL__</div><div class="value">__REVENUE__</div></div><div class="hero-stat"><div class="label">Members</div><div class="value">__MEMBERS__</div></div></div></div></section>
...
(async function(){
  ...
  try {
    ...
    return (html_template
        ...
        .replace('__METRIC_SOURCE__', 'MSSQL Admin Daily' if metric_source == 'admin_mssql' else 'GA4 + CRM Mart')
