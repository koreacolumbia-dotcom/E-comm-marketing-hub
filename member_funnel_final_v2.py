    ...
    def channel_panels(source: pd.DataFrame, mode: str, product_source_override: pd.DataFrame | None = None):
        ...
        for ch in channel_names(base_channels):
            ...
            if mode == "non_buyer":
                ...
                summary = {"buyers": int(max(sdf["member_id_norm"].replace("", pd.NA).nunique(), sdf["user_id_norm"].replace("", pd.NA).nunique())), "revenue": revenue, "orders": int(round(float(orders or 0))), "aov": (revenue / orders) if orders else 0.0, "avg_age": avg_age(sdf), "top_product": top_label(sdf, "purchase_product_name_norm"), "top_category": top_label(sdf, "top_category_norm")}
...
function focusCards(items, wide=false){ if(!items||!items.length) return '<div class="kpi-sub">데이터가 없습니다.</div>'; return items.slice(0,4).map(x=>{ const img=x.image_url||x.image||''; const name=x.product_name||x.product||'미분류'; const topAge=x.avg_age||x.top_age||'미확인'; const metricLine=wide?`${esc2(x.category||'미분류')} · Buyers ${num(x.buyers||0)} · Orders ${num(x.orders||0)} · Revenue ${money(x.revenue||0)}`:`${esc2(x.category||'미분류')} · Buyers ${num(x.buyers||0)} · Revenue ${money(x.revenue||0)}`; const subLine=wide?`주구매 연령 ${esc2(topAge)} · AOV ${money(x.aov||0)}`:`주구매 연령 ${esc2(topAge)}`; return `<div class="card"><div class="product-card"><div class="thumb ${img?'':'thumb-empty'}">${img?`<img class="thumb" src="${esc2(img)}" alt="">`:'NO IMG'}</div><div><div class="mini-title">${esc2(name)}</div><div class="stack-meta">${metricLine}</div><div class="stack-meta">${subLine}</div></div></div></div>` }).join(''); }
