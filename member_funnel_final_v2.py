    ...
    def channel_panels(source: pd.DataFrame, mode: str, product_source_override: pd.DataFrame | None = None):
        ...
        for ch in channel_names(base_channels):
            ...
            if mode == "non_buyer":
                ...
                summary = {"buyers": buyer_count, "revenue": revenue, "avg_age": avg_age(sdf), "top_product": top_label(sdf, "purchase_product_name_norm"), "orders": int(round(orders)), "aov": revenue/orders if orders else 0.0, "repeat_buyers": repeat_buyers, "avg_orders_per_buyer": (orders / buyer_count) if buyer_count else 0.0, "avg_rev_per_buyer": (revenue / buyer_count) if buyer_count else 0.0, "vip_buyers": vip_buyers, "avg_repurchase_score": float(pd.to_numeric(sdf.get("repurchase_30d_score", 0), errors="coerce").fillna(0).mean()) if not sdf.empty else 0.0, "avg_churn_score": float(pd.to_numeric(sdf.get("churn_60d_score", 0), errors="coerce").fillna(0).mean()) if not sdf.empty else 0.0, "avg_ltv_score": float(pd.to_numeric(sdf.get("ltv_score", 0), errors="coerce").fillna(0).mean()) if not sdf.empty else 0.0}
                ...
                summary = {"members": int(sdf["member_id_norm"].replace("", pd.NA).nunique()), "buyers": int(buyer_df(sdf)["member_id_norm"].replace("", pd.NA).nunique()), "revenue": revenue, "aov": revenue/orders if orders else 0.0, "avg_age": avg_age(sdf), "top_product": top_label(sdf, "purchase_product_name_norm"), "avg_repurchase_score": float(pd.to_numeric(sdf.get("repurchase_30d_score", 0), errors="coerce").fillna(0).mean()) if not sdf.empty else 0.0, "avg_churn_score": float(pd.to_numeric(sdf.get("churn_60d_score", 0), errors="coerce").fillna(0).mean()) if not sdf.empty else 0.0, "high_ltv_pct": float((pd.to_numeric(sdf.get("ltv_score", 0), errors="coerce").fillna(0) >= 0.8).mean() * 100.0) if not sdf.empty else 0.0, "top_action": top_label(sdf.assign(ml_action_label=sdf.get("ml_action_type", "").replace("", pd.NA).fillna("GENERAL")), "ml_action_label", "GENERAL")}
