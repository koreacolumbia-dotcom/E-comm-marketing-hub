...
def build_bundle(df: pd.DataFrame, start_date: dt.date, end_date: dt.date, period_key: str, period_label: str):
    ...
    total_rows = buyer_df(build_user_rows(df_order[df_order["member_id_norm"] != ""])) if not df_order.empty else pd.DataFrame()
...
@media (max-width:1380px){.hero-grid{grid-template-columns:1fr}.hero-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}.grid-4{grid-template-columns:repeat(2,minmax(0,1fr))}.grid-3{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media (max-width:960px){.page{padding:16px 14px 72px}.hero{padding:20px}.hero-kpis,.grid-4,.grid-3,.grid-2{grid-template-columns:1fr}.toolbar,.section-head{flex-direction:column;align-items:flex-start}.section-head h2{font-size:24px}.hero-copy h1{font-size:36px}.summary-card,.card{padding:20px}.chart-row{grid-template-columns:84px 1fr 56px}.donut-wrap{grid-template-columns:1fr}.donut{margin:0 auto}}
</style>
</head>
<body>
<div class="page">
...
function focusCards(items){ if(!items||!items.length) return '<div class="kpi-sub">데이터가 없습니다.</div>'; return items.slice(0,4).map(x=>`<div class="card"><div class="product-card"><div class="thumb ${x.image_url?'':'thumb-empty'}">${x.image_url?`<img class="thumb" src="${esc2(x.image_url)}" alt="">`:'NO IMG'}</div><div><div class="mini-title">${esc2(x.product_name||'미분류')}</div><div class="stack-meta">${esc2(x.category||'미분류')} · Buyers ${num(x.buyers||0)} · Revenue ${money(x.revenue||0)}</div></div></div></div>`).join(''); }
...
function renderNonBuyer(section){ const cards=`<div class="grid-4 nonbuyer-grid"><div class="card"><div class="kicker">Users</div><div class="kpi">${num(section.summary.users)}</div><div class="kpi-sub">구매 이력이 없는 로그인 유저</div></div><div class="card"><div class="kicker">Avg PV</div><div class="kpi">${Number(section.summary.avg_pageviews||0).toFixed(1)}</div><div class="kpi-sub">평균 페이지뷰</div></div><div class="card"><div class="kicker">Add To Cart</div><div class="kpi">${num(section.summary.add_to_cart_users)}</div><div class="kpi-sub">장바구니 담기 유저</div></div><div class="card"><div class="kicker">High Intent</div><div class="kpi">${num(section.summary.high_intent_users)}</div><div class="kpi-sub">PV/ATC 기반 고의도</div></div></div>`; return `<section><div class="section-head"><div><div class="section-title">NON BUYER</div><h2>비구매 유저 진단</h2></div></div>${cards}<div class="grid-2"><div class="card"><div class="section-title">CHANNEL MIX</div>${bar(section.channel_dist)}</div><div class="card"><div class="section-title">CATEGORY INTEREST</div>${bar(section.category_dist)}</div></div><div class="card">${table(section.rows,[['member_id','Member ID'],['user_id','USER_ID'],['channel_group','Channel'],['campaign','Campaign'],['pageviews','PV'],['add_to_cart','ATC'],['preferred_product','관심 상품']],['pageviews','add_to_cart'],'nb-table')}</div></section>`; }
function renderBuyer(section){ const tabs=subTabs('buyer', section.channels.map(x=>({key:x.channel,label:x.channel}))); const panels=section.channels.map((ch,i)=>`<div class="channel-panel ${i===0?'active':''}" data-panel-group="buyer" data-panel-id="buyer-${groupSlug(ch.channel)}" ${i===0?'':'hidden'}><div class="grid-4"><div class="card"><div class="kicker">Buyers</div><div class="kpi">${num(ch.summary.buyers)}</div></div><div class="card"><div class="kicker">Revenue</div><div class="kpi">${money(ch.summary.revenue)}</div></div><div class="card"><div class="kicker">Orders</div><div class="kpi">${num(ch.summary.orders)}</div></div><div class="card"><div class="kicker">AOV</div><div class="kpi">${money(ch.summary.aov)}</div></div></div><div class="grid-2"><div class="card"><div class="section-title">AGE MIX</div>${bar(ch.age_dist)}</div><div class="card"><div class="section-title">GENDER MIX</div>${donut(ch.gender_dist)}</div><div class="card"><div class="section-title">TOP CATEGORY</div>${bar(ch.category_dist)}</div><div class="card"><div class="section-title">TOP PRODUCTS</div><div class="grid-2">${focusCards(ch.products)}</div></div></div><div class="card">${table(ch.rows,[['member_id','Member ID'],['user_id','USER_ID'],['campaign','Campaign'],['purchase_product_name','구매 상품명'],['orders','Orders'],['revenue','Revenue']],['orders','revenue'],`buyer-${groupSlug(ch.channel)}`)}</div></div>`).join(''); return `<section><div class="section-head"><div><div class="section-title">BUYER</div><h2>채널별 구매 유저</h2></div>${tabs}</div>${panels}</section>`; }
function renderProduct(section){ return `<section><div class="section-head"><div><div class="section-title">PRODUCT</div><h2>구매 상품 집중 분석</h2></div></div><div class="grid-3">${section.groups.map(g=>`<div class="card"><div class="kicker">${esc2(g.label)}</div><div class="kpi">${num(g.buyers)}</div><div class="kpi-sub">Revenue ${money(g.revenue)} · Share ${pct(g.share)}</div><div class="kpi-sub" style="margin-top:14px">${esc2(g.description||'')}</div></div>`).join('')}</div><div class="card">${table(section.rows,[['channel_group','Channel'],['category','Category'],['product_name','Product'],['buyers','Buyers'],['revenue','Revenue']],['buyers','revenue'],'product-table')}</div></section>`; }
function renderTarget(section){ return `<section><div class="section-head"><div><div class="section-title">TARGET</div><h2>CRM 액션 대상 세그먼트</h2></div></div><div class="grid-4">${section.segments.map(seg=>`<div class="card"><div class="kicker">${esc2(seg.label)}</div><div class="kpi">${num(seg.users)}</div><div class="kpi-sub">${esc2(seg.message)}</div></div>`).join('')}</div><div class="card">${table(section.rows,[['segment','Segment'],['member_id','Member ID'],['phone','Phone'],['channel_group','Channel'],['campaign','Campaign'],['preferred_product','관심 상품'],['recommended_message','메시지']],[],'target-table')}</div></section>`; }
function renderTotal(section){ const tabs=subTabs('total', section.channels.map(x=>({key:x.channel,label:x.channel}))); const panels=section.channels.map((ch,i)=>`<div class="channel-panel ${i===0?'active':''}" data-panel-group="total" data-panel-id="total-${groupSlug(ch.channel)}" ${i===0?'':'hidden'}><div class="grid-4"><div class="card"><div class="kicker">Members</div><div class="kpi">${num(ch.summary.users)}</div></div><div class="card"><div class="kicker">Buyers</div><div class="kpi">${num(ch.summary.buyers)}</div></div><div class="card"><div class="kicker">Revenue</div><div class="kpi">${money(ch.summary.revenue)}</div></div><div class="card"><div class="kicker">대표 구매 상품명</div><div class="kpi" style="font-size:22px;line-height:1.2;font-family:'Pretendard','Noto Sans KR',sans-serif">${esc2(ch.summary.top_product)}</div></div></div><div class="grid-2"><div class="card"><div class="section-title">AGE 비율</div>${bar(ch.age_dist)}</div><div class="card"><div class="section-title">GENDER 비율</div>${donut(ch.gender_dist)}</div><div class="card"><div class="section-title">CATEGORY 비율</div>${bar(ch.category_dist)}</div><div class="card"><div class="section-title">구매 상품명 Focus</div><div class="grid-2">${focusCards(ch.products)}</div></div></div><div class="card">${table(ch.rows,[['member_id','Member ID'],['user_id','USER_ID'],['channel_group','Channel'],['campaign','Campaign'],['purchase_product_name','구매 상품명'],['orders','Orders'],['revenue','Revenue']],['orders','revenue'],`tot-${groupSlug(ch.channel)}`)}</div></div>`).join(''); return `<section><div class="section-head"><div><div class="section-title">TOTAL MEMBER</div><h2>기존 회원 전체 분석</h2></div>${tabs}</div>${panels}</section>`; }
function bindUi(){
