...
function renderProduct(section){ const channels=safePanelSet(section); const totalRevenue=channels.reduce((acc,ch)=>acc+Number(ch.summary?.revenue||0),0) || 0; const cards=channels.map(ch=>{ const share=totalRevenue ? Number(ch.summary?.revenue||0)/totalRevenue : 0; const topCat=(ch.category_dist||[])[0]?.label || '미분류'; return `<div class="card"><div class="kicker">${esc2(ch.channel)}</div><div class="kpi">${num(ch.summary?.buyers ?? 0)}</div><div class="kpi-sub">Revenue ${money(ch.summary?.revenue ?? 0)} · Share ${pct(share)}</div><div class="kpi-sub" style="margin-top:14px">Top Category ${esc2(topCat)}</div></div>` }).join(''); const rows=channels.flatMap(ch=>(ch.rows||[]).map(r=>({channel_group:r.channel||ch.channel, product_name:r.product||'', buyers:r.buyers||0, revenue:r.revenue||0}))); return `<section><div class="section-head"><div><div class="section-title">PRODUCT</div><h2>구매 상품 집중 분석</h2></div></div><div class="grid-3">${cards || '<div class="kpi-sub">데이터가 없습니다.</div>'}</div><div class="card">${table(rows,[['channel_group','Channel'],['product_name','Product'],['buyers','Buyers'],['revenue','Revenue']],['buyers','revenue'],'product-table')}</div></section>`; }
