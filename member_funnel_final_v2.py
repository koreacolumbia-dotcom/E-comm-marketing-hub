...
MSSQL_DAILY_SQL = os.getenv("MEMBER_FUNNEL_MSSQL_DAILY_SQL", "").strip()
MSSQL_DAILY_JSON = os.getenv("MEMBER_FUNNEL_MSSQL_DAILY_JSON", "").strip()
MSSQL_ADMIN_METRICS_LABEL = os.getenv("MEMBER_FUNNEL_MSSQL_ADMIN_METRICS_LABEL", "ADMIN").strip()
...
def get_mssql_connection():
    ...
    if not host or not database or not username:
        raise RuntimeError("MSSQL env vars are missing")
    conn_str = (
        f"DRIVER={{{driver}}};"
        f"SERVER={host},{port};"
        f"DATABASE={database};"
...
def load_admin_daily_metrics(target_date: dt.date) -> dict | None:
    if MSSQL_DAILY_JSON:
        p = Path(MSSQL_DAILY_JSON)
        if p.exists():
            try:
                data = json.loads(p.read_text(encoding='utf-8'))
                if isinstance(data, list):
                    row = next((x for x in data if str(x.get('date') or x.get('일자') or '')[:10] == target_date.isoformat()), None)
                else:
                    row = data
                ...
                pass
    if not MSSQL_DAILY_SQL:
        return None
    try:
        conn = get_mssql_connection()
        try:
            sql = MSSQL_DAILY_SQL.replace('__TARGET_DATE__', target_date.isoformat())
...
def coalesce_metric(d: dict | None, *keys: str, default=0):
    if not d:
        return default
...
def classify_category_from_name(name: str) -> str:
    n = (name or '').upper()
    if any(x in n for x in ['BACKPACK','PACKABLE BACKPACK','BAG','HIPSACK','SLING']): return 'BACKPACK'
    if any(x in n for x in ['BOOT','SHOE','SANDAL','SNEAKER','CRESTWOOD','KONOS','PEAKFREAK']): return 'SHOES'
    if any(x in n for x in ['PADDING','DOWN','PUFFER']): return 'PADDING'
    if any(x in n for x in ['FLEECE','플리스']): return 'FLEECE'
    if 'JACKET' in n or '자켓' in n or 'WINDBREAKER' in n: return 'JACKET'
    if 'PANTS' in n or 'SHORTS' in n or '팬츠' in n: return 'PANTS'
    if any(x in n for x in ['TEE','TSHIRT','T-SHIRT','SLEEVE','SHIRT']): return 'TOP'
    if any(x in n for x in ['CAP','HAT','BEANIE']): return 'HAT'
    if any(x in n for x in ['SOCK','GLOVE','BELT']): return 'ACCESSORY'
    return '미분류'
...
def normalize_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out.loc[out["top_category_norm"] == "", "top_category_norm"] = out.loc[out["top_category_norm"] == "", "purchase_product_name_norm"].map(classify_category_from_name)
...
def build_bundle(df: pd.DataFrame, start_date: dt.date, end_date: dt.date, period_key: str, period_label: str):
    ...
    admin_daily = load_admin_daily_metrics(latest_date) if period_key == '1d' else None
    ...
    bundle = {
        ...
        "latest_summary": [
            ...
            f"해당 일자 세션 {fmt_int(coalesce_metric(admin_daily, 'SESSION', 'sessions', 'session', default=latest['sessions_norm'].sum()))} / 가입 {fmt_int(coalesce_metric(admin_daily, '가입자 수', 'signups', 'signup', default=latest['signup_norm'].sum()))} / 구매자 {fmt_int(coalesce_metric(admin_daily, '구매 수량', 'buyers', 'buyer_count', default=latest['purchase_norm'].sum()))} 흐름입니다.",
        ...
        "overview": {"sessions": int(coalesce_metric(admin_daily, 'SESSION', 'sessions', 'session', default=df['sessions_norm'].sum()) if period_key == '1d' else df['sessions_norm'].sum()), "orders": int(coalesce_metric(admin_daily, '주문 건수 (주문번호)', '주문 건수', 'orders', default=df['orders_norm'].sum()) if period_key == '1d' else df['orders_norm'].sum()), "revenue": float(coalesce_metric(admin_daily, '총매출금액 (ERP매출)', '총매출금액', 'revenue', 'sales', default=df['revenue_norm'].sum()) if period_key == '1d' else df['revenue_norm'].sum()), "signups": int(coalesce_metric(admin_daily, '가입자 수', 'signups', 'signup', default=df['signup_norm'].sum()) if period_key == '1d' else df['signup_norm'].sum()), "buyers": int(coalesce_metric(admin_daily, '구매 수량', 'buyers', 'buyer_count', default=buy['member_id_norm'].replace('', pd.NA).nunique()) if period_key == '1d' else buy['member_id_norm'].replace('', pd.NA).nunique()), "members": int(user['member_id_norm'].replace('', pd.NA).nunique()), "metric_source": (MSSQL_ADMIN_METRICS_LABEL if admin_daily and period_key == '1d' else 'GA4+CRM')},
...
.grid-4{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:18px}.grid-3{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px}.grid-2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}.card{padding:22px; min-height:112px}.kpi-sub{margin-top:8px;font-size:12px;font-weight:700;color:var(--muted)}
...
<body>
<div class="page">
<section class="hero"><div class="hero-grid"><div><div class="eyebrow">External Signal Style · CRM Funnel</div><h1>Member Funnel</h1><p>채널별 행동 데이터와 CRM 액션 대상을 한 화면에서 빠르게 읽고, 바로 추출할 수 있게 재정렬했습니다. USER VIEW는 USER_ID 단위 대표행 기준 액션 분석, TOTAL VIEW는 기존 회원 전체 분석입니다.</p><div class="hero-meta">__PERIOD_NAV__</div></div><div class="hero-kpis"><div class="hero-stat"><div class="label">Sessions</div><div class="value">__SESSIONS__</div></div><div class="hero-stat"><div class="label">Buyers</div><div class="value">__BUYERS__</div></div><div class="hero-stat"><div class="label">Revenue</div><div class="value">__REVENUE__</div></div><div class="hero-stat"><div class="label">Members</div><div class="value">__MEMBERS__</div></div></div></div></section>
...
<div class="summary-card"><div class="section-title">이번 구간 핵심 요약</div><ul>__LATEST_SUMMARY__</ul></div>
