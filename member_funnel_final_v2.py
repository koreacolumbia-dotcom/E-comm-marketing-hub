...
def fmt_money(v: Any) -> str:
    try:
        return f"â©{int(round(float(v or 0))):,}"
    except Exception:
        return "â©0"
...
def normalize_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["age_band_norm"] = age_band.where(age_band.astype(str).str.strip()!="", derived.fillna("ë¯¸íì¸")).astype(str)
    ...
    out["gender_norm"] = gender.where(gender!="","ë¯¸íì¸")
    out["campaign_display_norm"] = [clean_label(a,"") or clean_label(b,"") or "ë¯¸ë¶ë¥" for a,b in zip(safe_series(out,["campaign_display","session_campaign","first_campaign","latest_campaign"],""), out["channel_group_norm"])]
...
def build_bundle(df: pd.DataFrame, start_date: dt.date, end_date: dt.date, period_key: str, period_label: str):
    ...
    def channel_panels(source: pd.DataFrame, mode: str):
        ...
        for ch in channel_names(source):
            ...
            prod_cards = [{"product": clean_label(r.get("purchase_product_name_norm"), "ë¯¸ë¶ë¥"), "category": clean_label(r.get("top_category_norm"), "ë¯¸ë¶ë¥"), "buyers": int(r.get("buyers",0)), "revenue": float(r.get("revenue",0)), "image": str(r.get("product_image_norm","") or "")} for _,r in products.head(8).iterrows()]
            if mode == "non_buyer":
                ...
                rows = [{"channel": clean_label(r.get("channel_group_norm"),"ë¯¸ë¶ë¥"), "product": clean_label(r.get("purchase_product_name_norm"),"ë¯¸ë¶ë¥"), "buyers": int(r.get("buyers",0)), "revenue": float(r.get("revenue",0))} for _,r in table.head(100).iterrows()]
    ...
    bundle = {
        ...
        "latest_summary": [
            f"ìµê·¼ ì í ê°ë¥ ì¼ì ê¸°ì¤ ìµì  ë°ì´í°ë {fmt_date(latest_date)}ìëë¤.",
            f"í´ë¹ ì¼ì ì¸ì {fmt_int(latest['sessions_norm'].sum())} / ê°ì {fmt_int(latest['signup_norm'].sum())} / êµ¬ë§¤ì {fmt_int(latest['purchase_norm'].sum())} íë¦ìëë¤.",
            f"ì£¼ì ì±ëì {top_label(latest, 'channel_group_norm')}, ë§¤ì¶ ê¸°ì¬ ìì ìíì {top_label(latest[latest['revenue_norm']>0], 'purchase_product_name_norm')}ìëë¤.",
        ],
...
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:Inter,Arial,sans-serif}.page{max-width:1320px;margin:0 auto;padding:28px}
.hero{background:linear-gradient(135deg,var(--navy),var(--blue));border-radius:32px;padding:34px;color:#fff;box-shadow:0 24px 50px rgba(15,37,95,.18)}
.hero-grid{display:grid;grid-template-columns:1.4fr 1fr;gap:24px;align-items:stretch}.eyebrow{display:inline-flex;padding:8px 12px;border:1px solid rgba(255,255,255,.25);border-radius:999px;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.12em;background:rgba(255,255,255,.08)}
...
.hero-kpis{display:grid;grid-template-columns:repeat(2,1fr);gap:14px}.hero-stat,.card,.summary-card{background:var(--card);border:1px solid var(--line);border-radius:24px;box-shadow:0 10px 26px rgba(15,23,42,.05)}.hero-stat{padding:18px 20px;color:var(--text)}.label,.kicker{font-size:11px;font-weight:900;letter-spacing:.16em;text-transform:uppercase;color:var(--muted)}.value,.kpi{margin-top:10px;font-size:28px;font-weight:1000;line-height:1.1;word-break:break-word}.toolbar{display:flex;justify-content:space-between;align-items:center;gap:12px;margin:28px 0 18px}.tabs,.subtabs{display:flex;gap:10px;flex-wrap:wrap}.tab-btn,.subtab-btn,.table-expand-btn{height:40px;padding:0 16px;border-radius:999px;border:1px solid var(--line);background:#fff;color:var(--navy);font-weight:900;cursor:pointer}.tab-btn.active,.subtab-btn.active{background:var(--navy);color:#fff;border-color:var(--navy)}.summary-card{padding:18px 22px;margin-bottom:22px}.summary-card ul{margin:8px 0 0 16px;padding:0}.summary-card li{margin:8px 0;font-weight:700}
.panel{display:none}.panel.active{display:block}.section-head{display:flex;justify-content:space-between;align-items:flex-end;gap:12px;margin:26px 0 14px}.section-title{font-size:12px;font-weight:900;letter-spacing:.18em;text-transform:uppercase;color:#64748b}.section-head h2{margin:6px 0 0;font-size:24px;line-height:1.2}
.download-row{display:flex;gap:8px;flex-wrap:wrap}.download-btn{display:inline-flex;align-items:center;height:40px;padding:0 14px;border-radius:999px;background:var(--navy);color:#fff;text-decoration:none;font-weight:900}
.grid-4{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.grid-3{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.grid-2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.card{padding:18px}.kpi-sub{margin-top:8px;font-size:12px;font-weight:700;color:var(--muted)}
...
.product-card{display:flex;gap:12px;align-items:center}.thumb{width:44px;height:44px;border-radius:12px;object-fit:cover;border:1px solid var(--line)}.thumb-empty{display:flex;align-items:center;justify-content:center;background:#eef3f8;color:#64748b;font-size:11px;font-weight:900}.mini-title{margin-top:4px;font-size:15px;font-weight:900}.stack-meta{margin-top:6px;font-size:12px;font-weight:800;color:var(--muted)}
.table-meta,.table-tools{display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;font-size:12px;font-weight:800;color:var(--muted)}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:18px}.data-table{width:100%;min-width:860px;border-collapse:collapse}th,td{padding:12px 10px;border-bottom:1px solid #eef3f8;text-align:left;font-size:12px;font-weight:800;white-space:nowrap}th{background:#f8fbff;color:#64748b;text-transform:uppercase;letter-spacing:.08em;font-size:11px}td.num,th.num{text-align:right}.is-hidden{display:none}
@media (max-width:1100px){.hero-grid,.grid-4,.grid-3,.grid-2{grid-template-columns:1fr}h1{font-size:40px}}
</style>
</head>
<body>
<div class="page">
<section class="hero"><div class="hero-grid"><div><div class="eyebrow">External Signal Style Â· CRM Funnel</div><h1>Member Funnel</h1><p>ì±ëë³ íë ë°ì´í°ì CRM ì¡ì ëìì í íë©´ìì ë¹ ë¥´ê² ì½ê³ , ë°ë¡ ì¶ì¶í  ì ìê² ì¬ì ë ¬íìµëë¤. USER VIEWë USER_ID ë¨ì ëíí ê¸°ì¤ ì¡ì ë¶ì, TOTAL VIEWë ê¸°ì¡´ íì ì ì²´ ë¶ììëë¤.</p><div class="hero-meta">__PERIOD_NAV__</div></div><div class="hero-kpis"><div class="hero-stat"><div class="label">Sessions</div><div class="value">__SESSIONS__</div></div><div class="hero-stat"><div class="label">Buyers</div><div class="value">__BUYERS__</div></div><div class="hero-stat"><div class="label">Revenue</div><div class="value">__REVENUE__</div></div><div class="hero-stat"><div class="label">Members</div><div class="value">__MEMBERS__</div></div></div></div></section>
...
<div class="summary-card"><div class="section-title">ì´ë² êµ¬ê° íµì¬ ìì½</div><ul>__LATEST_SUMMARY__</ul></div>
<section class="panel active" id="user-view"><div class="section-head"><div><div class="section-title">USER VIEW</div><h2>íë ë°ì´í° Â· CRM ì¡ì ë·° (USER_ID ëíí ê¸°ì¤)</h2></div><div class="download-row">__DOWNLOADS__</div></div><div id="user-sections"></div></section>
<section class="panel" id="total-view"><div class="section-head"><div><div class="section-title">TOTAL VIEW</div><h2>ê¸°ì¡´ íì ì ì²´ ë¶ì</h2></div></div><div id="total-sections"></div></section>
</div>
<script>
const BUNDLE = __DATA_JSON__;
function money(v){const n=Number(v||0); return 'â©'+Math.round(n).toLocaleString('ko-KR')}
function num(v){return Math.round(Number(v||0)).toLocaleString('ko-KR')}
function pct(v){return `${Number(v||0).toFixed(1)}%`}
function esc2(s){return String(s ?? '').replace(/[&<>"']/g, m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}
function bar(items){ if(!items||!items.length) return '<div class="kpi-sub">ë°ì´í°ê° ììµëë¤.</div>'; const max=Math.max(...items.map(x=>Number(x.count||0)),1); return items.map(x=>`<div class="chart-row"><div class="chart-label">${esc2(x.label)}</div><div class="chart-track"><span class="chart-fill" style="width:${(Number(x.count||0)/max*100).toFixed(1)}%"></span></div><div class="chart-metric">${pct(x.share_pct)}</div></div>`).join('') }
function donut(items){ if(!items||!items.length) return '<div class="kpi-sub">ë°ì´í°ê° ììµëë¤.</div>'; const a=Number(items[0]?.share_pct||0), b=Number(items[1]?.share_pct||Math.max(0,100-a)); const bg=`conic-gradient(#2563eb 0 ${a}%, #93c5fd ${a}% ${a+b}%, #e2e8f0 ${a+b}% 100%)`; const lg=items.slice(0,2).map((x,i)=>`<div class="legend-item"><span><span class="legend-dot ${i?'alt':''}"></span>${esc2(x.label)}</span><strong>${pct(x.share_pct)}</strong></div>`).join(''); return `<div class="donut-wrap"><div class="donut" style="background:${bg}"></div><div>${lg}</div></div>`; }
function table(rows, cols, numeric, id){ const limit=15; const head=cols.map(c=>`<th class="${numeric.includes(c[0])?'num':''}">${esc2(c[1])}</th>`).join(''); const body=(rows||[]).map((r,i)=>`<tr class="${i>=limit?'is-hidden extra-row':''}">${cols.map(c=>{let v=r[c[0]] ?? ''; if(numeric.includes(c[0])) v=(c[0].includes('revenue')||c[0]==='aov')?money(v):num(v); return `<td class="${numeric.includes(c[0])?'num':''}">${esc2(v)}</td>`;}).join('')}</tr>`).join('') || `<tr><td colspan="${cols.length}">ë°ì´í°ê° ììµëë¤.</td></tr>`; const btn=(rows||[]).length>limit?`<button class="table-expand-btn" data-expand="${id}">ì ì²´ë³´ê¸°</button>`:''; return `<div class="table-meta"><span>ì ì²´ ${num((rows||[]).length)}í ì¤ ${num(Math.min((rows||[]).length,15))}í ê¸°ë³¸ íì</span><span>${btn}</span></div><div class="table-wrap"><table class="data-table" id="${id}"><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table></div>`; }
function focusCards(items){ if(!items||!items.length) return '<div class="kpi-sub">ë°ì´í°ê° ììµëë¤.</div>'; return items.slice(0,4).map(x=>`<div class="card product-card"><div>${x.image?`<img class="thumb" src="${esc2(x.image)}" alt="">`:'<div class="thumb thumb-empty">IMG</div>'}</div><div><div class="kicker">${esc2(x.category||'ë¯¸ë¶ë¥')}</div><div class="mini-title">${esc2(x.product||'ë¯¸ë¶ë¥')}</div><div class="stack-meta">Buyers ${num(x.buyers)} / Revenue ${money(x.revenue)}</div></div></div>`).join(''); }
...
function groupSlug(s){ return String(s).toLowerCase().replace(/[^a-z0-9ê°-í£]+/g,'-'); }
function renderNonBuyer(block){ return `<div class="section-head"><div><div class="section-title">NON BUYER</div><h2>ê°ìíì§ë§ ìì§ ì¬ì§ ìì ì¬ë</h2></div></div>${tabsHtml('nonbuyer', block.channels)}${block.channels.map((ch,i)=>{ const p=block.panels[ch]; return `<div class="channel-panel ${i===0?'active':''}" data-panel-group="nonbuyer" data-panel-id="${ch}" ${i===0?'':'hidden'}><div class="grid-4"><div class="card"><div class="kicker">Non Buyer Members</div><div class="kpi">${num(p.summary.members)}</div></div><div class="card"><div class="kicker">Non Buyer Sessions</div><div class="kpi">${num(p.summary.sessions)}</div></div><div class="card"><div class="kicker">íê·  ëì´</div><div class="kpi">${p.summary.avg_age||'-'}</div></div><div class="card"><div class="kicker">ëí ì ì ì±ë</div><div class="kpi">${esc2(p.summary.top_channel)}</div></div></div><div class="grid-3"><div class="card"><div class="section-title">AGE ë¹ì¨</div>${bar(p.age_dist)}</div><div class="card"><div class="section-title">GENDER ë¹ì¨</div>${donut(p.gender_dist)}</div><div class="card"><div class="section-title">ëí ì ì ì±ë ë¹ì¨</div>${bar(p.channel_dist)}</div></div><div class="card">${table(p.rows,[['member_id','Member ID'],['phone','Phone'],['user_id','USER_ID'],['channel_group','Channel'],['campaign','Campaign'],['sessions','Sessions'],['pageviews','PV']],['sessions','pageviews'],`nb-${groupSlug(ch)}`)}</div></div>`; }).join('')}`; }
function renderBuyer(block){ return `<div class="section-head"><div><div class="section-title">BUYER REVENUE</div><h2>ëê° ë§¤ì¶ì ë§ë¤ìëì§</h2></div></div>${tabsHtml('buyer', block.channels)}${block.channels.map((ch,i)=>{ const p=block.panels[ch]; return `<div class="channel-panel ${i===0?'active':''}" data-panel-group="buyer" data-panel-id="${ch}" ${i===0?'':'hidden'}><div class="grid-4"><div class="card"><div class="kicker">Buyers</div><div class="kpi">${num(p.summary.buyers)}</div></div><div class="card"><div class="kicker">Revenue</div><div class="kpi">${money(p.summary.revenue)}</div></div><div class="card"><div class="kicker">AOV</div><div class="kpi">${money(p.summary.aov)}</div></div><div class="card"><div class="kicker">ëí êµ¬ë§¤ ìíëª</div><div class="kpi">${esc2(p.summary.top_product)}</div></div></div><div class="grid-2"><div class="card"><div class="section-title">AGE ë¹ì¨</div>${bar(p.age_dist)}</div><div class="card"><div class="section-title">GENDER ë¹ì¨</div>${donut(p.gender_dist)}</div></div><div class="card">${table(p.rows,[['member_id','Member ID'],['user_id','USER_ID'],['channel_group','Channel'],['campaign','Campaign'],['purchase_product_name','êµ¬ë§¤ ìíëª'],['orders','Orders'],['revenue','Revenue']],['orders','revenue'],`buy-${groupSlug(ch)}`)}</div></div>`; }).join('')}`; }
function renderProduct(block){ return `<div class="section-head"><div><div class="section-title">PRODUCT INSIGHT</div><h2>ë¬´ì¨ ìíì´ ê³ ê°ì ìì§ìëì§</h2></div></div>${tabsHtml('product', block.channels)}${block.channels.map((ch,i)=>{ const p=block.panels[ch]; return `<div class="channel-panel ${i===0?'active':''}" data-panel-group="product" data-panel-id="${ch}" ${i===0?'':'hidden'}><div class="grid-2"><div class="card"><div class="section-title">Category ë¹ì¨</div>${bar(p.category_dist)}</div><div class="card"><div class="section-title">êµ¬ë§¤ ìíëª Focus</div><div class="grid-2">${focusCards(p.products)}</div></div></div><div class="card">${table(p.rows,[['channel','Channel'],['product','Product'],['buyers','Buyers'],['revenue','Revenue']],['buyers','revenue'],`prod-${groupSlug(ch)}`)}</div></div>`; }).join('')}`; }
function renderTarget(target){ return `<div class="section-head"><div><div class="section-title">ì§ê¸ ë°ë¡ ì¡ì ê°ë¥í ëìì</div><h2>ì¸ê·¸ë¨¼í¸ë³ ì±ë Â· ê´ì¬ìí Â· ì¶ì² ë©ìì§</h2></div></div><div class="grid-2">${(target.cards||[]).map(x=>`<div class="card"><div class="kicker">${esc2(x.label)}</div><div class="kpi">${num(x.count)}ëª</div><div class="stack-meta">Top Channel Â· ${esc2(x.top_channel)}</div><div class="stack-meta">Top Product Â· ${esc2(x.top_product)}</div><div class="stack-meta">Message Â· ${esc2(x.top_message)}</div></div>`).join('')}</div>`; }
function renderTotal(block){ return `${tabsHtml('totalmember', block.channels)}${block.channels.map((ch,i)=>{ const p=block.panels[ch]; return `<div class="channel-panel ${i===0?'active':''}" data-panel-group="totalmember" data-panel-id="${ch}" ${i===0?'':'hidden'}><div class="grid-4"><div class="card"><div class="kicker">ê¸°ì¡´ íì</div><div class="kpi">${num(p.summary.members)}</div></div><div class="card"><div class="kicker">êµ¬ë§¤ íì</div><div class="kpi">${num(p.summary.buyers)}</div></div><div class="card"><div class="kicker">Revenue</div><div class="kpi">${money(p.summary.revenue)}</div></div><div class="card"><div class="kicker">ëí êµ¬ë§¤ ìíëª</div><div class="kpi">${esc2(p.summary.top_product)}</div></div></div><div class="grid-2"><div class="card"><div class="section-title">AGE ë¹ì¨</div>${bar(p.age_dist)}</div><div class="card"><div class="section-title">GENDER ë¹ì¨</div>${donut(p.gender_dist)}</div><div class="card"><div class="section-title">Category ë¹ì¨</div>${bar(p.category_dist)}</div><div class="card"><div class="section-title">êµ¬ë§¤ ìíëª Focus</div><div class="grid-2">${focusCards(p.products)}</div></div></div><div class="card">${table(p.rows,[['member_id','Member ID'],['user_id','USER_ID'],['channel_group','Channel'],['campaign','Campaign'],['purchase_product_name','êµ¬ë§¤ ìíëª'],['orders','Orders'],['revenue','Revenue']],['orders','revenue'],`tot-${groupSlug(ch)}`)}</div></div>`; }).join('')}`; }
...
document.addEventListener('click', (e)=>{ const sub=e.target.closest('.subtab-btn'); if(sub){ const g=sub.dataset.group, t=sub.dataset.target; document.querySelectorAll(`.subtab-btn[data-group="${g}"]`).forEach(b=>b.classList.toggle('active', b===sub)); document.querySelectorAll(`.channel-panel[data-panel-group="${g}"]`).forEach(p=>{ const on=p.dataset.panelId===t; p.hidden=!on; p.classList.toggle('active', on); }); } const ex=e.target.closest('[data-expand]'); if(ex){ const table=document.getElementById(ex.dataset.expand); if(table){ table.querySelectorAll('.extra-row').forEach(r=>r.classList.toggle('is-hidden')); ex.textContent = ex.textContent==='ì ì²´ë³´ê¸°' ? 'ì ê¸°' : 'ì ì²´ë³´ê¸°'; } } });
</script></body></html>"""
    download_html = ''
