...
def build_bundle(df: pd.DataFrame, start_date: dt.date, end_date: dt.date, period_key: str, period_label: str):
    ...
    def channel_panels(source: pd.DataFrame, mode: str):
        ...
        for ch in channel_names(src_dedup):
            ...
            product_source = sdf[sdf["purchase_product_name_norm"] != ""].copy() if not sdf.empty else pd.DataFrame()
            top_age_map = {}
            if not product_source.empty:
                age_rank = (
                    product_source[product_source["age_band_norm"] != ""]
                    .groupby(["purchase_product_name_norm", "age_band_norm"], dropna=False)
                    .agg(buyers=("member_id_norm", "nunique"))
                    .reset_index()
                    .sort_values(["purchase_product_name_norm", "buyers", "age_band_norm"], ascending=[True, False, True])
                )
                for product_name, g in age_rank.groupby("purchase_product_name_norm", dropna=False):
                    if not g.empty:
                        top_age_map[str(product_name)] = clean_label(g.iloc[0].get("age_band_norm"), "미확인")
            products = (
                product_source
            ...
            ) if not product_source.empty else pd.DataFrame()
            prod_cards = [{"product": clean_label(r.get("purchase_product_name_norm"), "미분류"), "category": clean_label(r.get("top_category_norm"), "미분류"), "code": clean_label(r.get("product_code_norm"), "-"), "buyers": int(r.get("buyers",0)), "revenue": float(r.get("revenue",0)), "top_age": top_age_map.get(str(r.get("purchase_product_name_norm") or ""), "미확인"), "image": normalize_image_url(r.get("image","") or "")} for _,r in products.head(8).iterrows()]
            if mode == "non_buyer":
                ...
                summary = {"buyers": int(max(sdf["member_id_norm"].replace("", pd.NA).nunique(), sdf["user_id_norm"].replace("", pd.NA).nunique())), "revenue": revenue, "orders": int(round(float(orders or 0))), "aov": revenue/orders if orders else 0.0, "avg_age": avg_age(sdf), "top_product": top_label(sdf, "purchase_product_name_norm")}
                ...
                table = product_source.groupby(["purchase_product_name_norm","top_category_norm","product_code_norm"], dropna=False).agg(buyers=("member_id_norm","nunique"), revenue=("metric_revenue_norm","sum")).reset_index().sort_values(["revenue","buyers"], ascending=[False,False]) if not product_source.empty else pd.DataFrame()
                rows = [{"product": clean_label(r.get("purchase_product_name_norm"),"미분류"), "category": clean_label(r.get("top_category_norm"),"미분류"), "product_code": clean_label(r.get("product_code_norm"),"-"), "top_age": top_age_map.get(str(r.get("purchase_product_name_norm") or ""), "미확인"), "buyers": int(r.get("buyers",0)), "revenue": float(r.get("revenue",0))} for _,r in table.head(100).iterrows()]
                summary = {"buyers": int(sdf["member_id_norm"].replace("", pd.NA).nunique()), "revenue": revenue, "avg_age": avg_age(sdf), "top_category": top_label(sdf, "top_category_norm"), "top_product": top_label(sdf, "purchase_product_name_norm")}
                extra = {"products": prod_cards}
...
function focusCards(items, wide=false){ if(!items||!items.length) return '<div class="kpi-sub">데이터가 없습니다.</div>'; return items.slice(0, wide?6:4).map(x=>`<div class="card product-card"><div>${x.image?`<img class="thumb" src="${esc2(x.image)}" alt="">`:'<div class="thumb thumb-empty">IMG</div>'}</div><div><div class="kicker">${esc2(x.category||'미분류')}</div><div class="mini-title">${esc2(x.product||'미분류')}</div><div class="stack-meta">Buyers ${num(x.buyers)} / Revenue ${money(x.revenue)}</div><div class="stack-meta">Top Age ${esc2(x.top_age||'-')} / Code ${esc2(x.code||'-')}</div></div></div>`).join(''); }
...
function renderBuyer(block){ return `<div class="section-head"><div><div class="section-title">BUYER REVENUE</div><h2>누가 매출을 만들었는지</h2><div class="kpi-sub" style="margin-top:8px">채널 디테일은 구매일 기준 USER_ID 대표행으로 집계합니다.</div></div></div>${tabsHtml('buyer', block.channels)}${block.channels.map((ch,i)=>{ const p=block.panels[ch]; return `<div class="channel-panel ${i===0?'active':''}" data-panel-group="buyer" data-panel-id="${ch}" ${i===0?'':'hidden'}><div class="grid-4"><div class="card"><div class="kicker">Buyers</div><div class="kpi">${num(p.summary.buyers)}</div></div><div class="card"><div class="kicker">Revenue</div><div class="kpi">${money(p.summary.revenue)}</div></div><div class="card"><div class="kicker">Orders</div><div class="kpi">${num(p.summary.orders)}</div></div></div><div class="grid-2"><div class="card"><div class="section-title">GENDER 비율</div>${donut(p.gender_dist)}</div><div class="card"><div class="kicker">AOV</div><div class="kpi">${money(p.summary.aov)}</div></div></div><div class="card full-span"><div class="section-title">TOP PRODUCTS</div><div class="stack-grid-2">${focusCards(p.products,true)}</div></div><div class="grid-2"><div class="card"><div class="section-title">AGE 비율</div>${bar(p.age_dist)}</div><div class="card">${table(p.rows,[['member_id','Member ID'],['user_id','USER_ID'],['channel_group','Channel'],['campaign','Campaign'],['purchase_product_name','구매 상품명'],['orders','Orders'],['revenue','Revenue']],['orders','revenue'],`buy-${groupSlug(ch)}`)}</div></div></div>`; }).join('')}`; }
function renderProduct(block){ return `<div class="section-head"><div><div class="section-title">PRODUCT INSIGHT</div><h2>구매 상품 집중 분석</h2><div class="kpi-sub" style="margin-top:8px">채널별로 어떤 상품을 누가 샀는지, 그리고 어떤 연령대가 중심인지 바로 확인할 수 있게 재정렬했습니다.</div></div></div>${tabsHtml('product', block.channels)}${block.channels.map((ch,i)=>{ const p=block.panels[ch]; return `<div class="channel-panel ${i===0?'active':''}" data-panel-group="product" data-panel-id="${ch}" ${i===0?'':'hidden'}><div class="grid-4"><div class="card"><div class="kicker">Revenue</div><div class="kpi">${money(p.summary.revenue)}</div></div><div class="card"><div class="kicker">Buyers</div><div class="kpi">${num(p.summary.buyers)}</div></div><div class="card"><div class="kicker">Top Category</div><div class="kpi">${esc2(p.summary.top_category||'-')}</div></div><div class="card"><div class="kicker">Top Product</div><div class="kpi">${esc2(p.summary.top_product||'-')}</div></div></div><div class="card full-span"><div class="section-title">TOP PRODUCTS</div><div class="stack-grid-2">${focusCards(p.products,true)}</div></div><div class="card">${table(p.rows,[['product','Product'],['category','Category'],['product_code','Product Code'],['top_age','Top Age'],['buyers','Buyers'],['revenue','Revenue']],['buyers','revenue'],`prod-${groupSlug(ch)}`)}</div></div>`; }).join('')}`; }
