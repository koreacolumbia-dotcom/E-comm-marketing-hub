...
def build_bundle(df: pd.DataFrame, start_date: dt.date, end_date: dt.date, period_key: str, period_label: str):
    ...
    member_activity = raw[
        (
            ((raw["login_date_norm"] >= start_date) & (raw["login_date_norm"] <= end_date))
            | ((raw["order_date_norm"] >= start_date) & (raw["order_date_norm"] <= end_date))
        )
        & (raw["member_id_norm"] != "")
    ].copy()
