...
def build_bundle(df: pd.DataFrame, start_date: dt.date, end_date: dt.date, period_key: str, period_label: str):
    ...
    bundle = {
        ...
        "overview": {"sessions": int(df['sessions_norm'].sum()), "orders": int(df['orders_norm'].sum()), "revenue": float(df['metric_revenue_norm'].sum()), "signups": int(df['signup_norm'].sum()), "buyers": int(buy['member_id_norm'].replace('', pd.NA).nunique()), "members": int(user['member_id_norm'].replace('', pd.NA).nunique()), "non_buyers": int(nb['member_id_norm'].replace('', pd.NA).nunique()), "metric_source": "ga4_crm_mart"},
...
*{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:#ffffff;color:var(--ink);font-family:'Inter','Pretendard','Noto Sans KR','Apple SD Gothic Neo','Malgun Gothic',Arial,sans-serif}.page{max-width:1640px;margin:0 auto;padding:28px 22px 96px}
.hero{position:relative;overflow:hidden;background:linear-gradient(180deg,#020717 0%,#030a1f 100%);border:1px solid rgba(15,23,42,.10);border-radius:22px;padding:28px 24px 14px;color:#fff;box-shadow:0 8px 24px rgba(2,6,23,.12)}.hero:before{content:'';position:absolute;inset:auto -120px -160px auto;width:360px;height:360px;border-radius:999px;background:radial-gradient(circle,rgba(37,99,235,.18),transparent 62%)}.hero:after{content:'';position:absolute;inset:-120px auto auto -120px;width:260px;height:260px;border-radius:999px;background:radial-gradient(circle,rgba(255,255,255,.06),transparent 68%)}
.hero-grid{position:relative;z-index:1;display:grid;grid-template-columns:minmax(0,1.42fr) minmax(460px,.96fr);gap:52px;align-items:stretch}.eyebrow{display:inline-flex;align-items:center;gap:8px;padding:6px 11px;border:1px solid rgba(255,255,255,.12);border-radius:999px;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.14em;background:rgba(255,255,255,.06);backdrop-filter:blur(8px)}h1{margin:16px 0 10px;font-size:38px;line-height:1.02;letter-spacing:-.04em;font-weight:900}.hero p{max-width:760px;margin:0 0 14px;font-size:12px;font-weight:800;line-height:1.75;opacity:.92}.hero-meta{display:flex;gap:10px;flex-wrap:wrap}.period-chip{display:inline-flex;align-items:center;justify-content:center;min-width:72px;height:34px;padding:0 16px;border-radius:999px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.04);color:#fff;text-decoration:none;font-size:12px;font-weight:900;backdrop-filter:blur(8px);transition:.18s ease}.period-chip:hover{transform:translateY(-1px);background:rgba(255,255,255,.10)}.period-chip.active{background:#fff;color:#0f172a;border-color:#fff;box-shadow:none}
...
.toolbar{display:flex;justify-content:space-between;align-items:center;gap:28px;margin:36px 0 30px}.tabs,.subtabs{display:flex;gap:10px;flex-wrap:wrap}.tab-btn,.subtab-btn,.table-expand-btn{height:32px;padding:0 14px;border-radius:10px;border:1px solid #e5e7eb;background:#f3f4f6;color:#475569;font-weight:900;cursor:pointer;box-shadow:none;transition:.18s ease;font-size:11px}.tab-btn:hover,.subtab-btn:hover,.table-expand-btn:hover{transform:none;border-color:#d1d5db}.tab-btn.active,.subtab-btn.active{background:#3b82f6;color:#fff;border-color:#3b82f6;box-shadow:none}
...
.table-meta,.table-tools{display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;font-size:12px;font-weight:800;color:var(--muted)}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:24px;background:#fff;box-shadow:0 18px 32px rgba(15,23,42,.06)}.data-table{width:100%;min-width:860px;border-collapse:collapse}th,td{padding:10px 10px;border-bottom:1px solid #f1f5f9;text-align:left;font-size:11px;font-weight:800;white-space:nowrap}th{background:#f8fafc;color:#64748b;text-transform:uppercase;letter-spacing:.08em;font-size:10px;position:sticky;top:0}td.num,th.num{text-align:right}.is-hidden{display:none}
.channel-panel{animation:fadeInUp .24s ease}.channel-panel[hidden]{display:none!important}@keyframes fadeInUp{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}.hero,.summary-card,.card,.table-wrap{animation:fadeInUp .22s ease}
