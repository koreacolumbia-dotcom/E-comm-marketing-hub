...
function renderTarget(section){ const cards=(section?.cards||[]).map((seg,idx)=>`<button class="card target-card ${idx===0?'active':''}" data-target-seg="${esc2(seg.key)}"><div class="kicker">${esc2(seg.label)}</div><div class="kpi">${num(seg.count)}</div><div class="kpi-sub">Top Channel ${esc2(seg.top_channel||'미분류')}</div><div class="kpi-sub" style="margin-top:8px">Top Msg ${esc2(seg.top_message||'GENERAL')}</div></button>`).join(''); const panels=(section?.cards||[]).map((seg,idx)=>{ const rows=(section?.segments?.[seg.key]?.rows)||[]; const dl=(BUNDLE?.downloads?.target_segments||{})[seg.key]||''; const head=dl?`<div class="section-head" style="margin-bottom:12px"><div></div><a class="download-btn" href="${esc2(dl)}">${esc2(seg.label)} Excel</a></div>`:''; return `<div class="target-panel ${idx===0?'active':''}" data-target-panel="${esc2(seg.key)}" ${idx===0?'':'hidden'}>${head}<div class="card">${table(rows,[['member_id','Member ID'],['phone','Phone'],['channel_group','Channel'],['campaign','Campaign'],['preferred_product','관심 상품'],['recommended_message','메시지']],[],'target-table-'+esc2(seg.key))}</div></div>`; }).join(''); return `<section><div class="section-head"><div><div class="section-title">TARGET</div><h2>CRM 액션 대상 세그먼트</h2></div></div><div class="grid-4">${cards || '<div class="kpi-sub">데이터가 없습니다.</div>'}</div>${panels}</section>`; }</div><div class="kpi">${num(seg.count)}</div><div class="kpi-sub">Top Channel ${esc2(seg.top_channel||'미분류')}</div><div class="kpi-sub" style="margin-top:8px">Top Msg ${esc2(seg.top_message||'GENERAL')}</div></div>`).join(''); return `<section><div class="section-head"><div><div class="section-title">TARGET</div><h2>CRM 액션 대상 세그먼트</h2></div></div><div class="grid-4">${cards || '<div class="kpi-sub">데이터가 없습니다.</div>'}</div><div class="card">${table(section?.rows||[],[['member_id','Member ID'],['phone','Phone'],['channel_group','Channel'],['campaign','Campaign'],['preferred_product','관심 상품'],['recommended_message','메시지']],[],'target-table')}</div></section>`; }
