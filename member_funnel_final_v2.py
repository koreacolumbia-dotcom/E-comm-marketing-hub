...
ML_SCORE_TABLE = "columbia-ga4.crm_mart.crm_member_totalview_scores"
...
def build_analysis_target_payload(user: pd.DataFrame) -> dict:
    work = _compute_driver_scores_from_frame(user.copy())
    if work.empty:
        return {"cards": [], "rows": []}
    if "consent_norm" in work.columns:
        base = work[work["consent_norm"].astype(float) >= 1].copy()
        if base.empty:
            base = work.copy()
    else:
        base = work.copy()
    score_cols = ["repurchase_30d_score", "first_purchase_30d_score", "churn_60d_score", "ltv_score"]
    base["driver_score"] = base[score_cols].max(axis=1)
    action_map = [
    ...
    cards = []
    for action, label in action_map:
        sdf = base[base["ml_action_type"].astype(str) == action].copy()
        if sdf.empty:
            continue
        cards.append({
            "label": label,
            "count": int(sdf["member_id_norm"].replace('', pd.NA).where(sdf["member_id_norm"].astype(str) != '', sdf.get("user_id_norm", '')).replace('', pd.NA).nunique()),
            "top_channel": top_label(sdf, "channel_group_norm", "미분류"),
            "top_product": top_label(sdf, "purchase_product_name_norm", "미분류"),
            "top_message": f"{action} · driver:{top_label(sdf, 'dominant_driver', 'general_signal')}",
        })
    rows = rows_from_df(
        base.sort_values(["driver_score", "ltv_score", "repurchase_30d_score"], ascending=[False, False, False]),
        {
            "member_id_norm":"member_id","user_id_norm":"user_id","phone_norm":"phone","channel_group_norm":"channel_group",
            "campaign_display_norm":"campaign","purchase_product_name_norm":"product","top_category_norm":"top_category",
            "dominant_driver":"dominant_driver","ml_action_type":"recommended_action","ml_priority_tier":"priority_tier",
            "repurchase_30d_score":"repurchase_score","first_purchase_30d_score":"first_buy_score",
            "churn_60d_score":"churn_score","ltv_score":"ltv_score","next_best_category":"next_best_category","consent_norm":"consent"
        }
    )
...
def fetch_purchase_journey_summary_bq(start_date: dt.date, end_date: dt.date) -> dict[str, float]:
    ...
    try:
        ...
        return {
            ...
            'first_purchase_top_category': str(row.get('first_purchase_top_category') or ''),
            'first_purchase_top_category_count': int(pd.to_numeric(row.get('first_purchase_top_category_count', 0), errors='coerce') or 0),
            'second_purchase_top_category': str(row.get('second_purchase_top_category') or ''),
            'second_purchase_top_category_count': int(pd.to_numeric(row.get('second_purchase_top_category_count', 0), errors='coerce') or 0),
            'third_purchase_top_category': str(row.get('third_purchase_top_category') or ''),
            'third_purchase_top_category_count': int(pd.to_numeric(row.get('third_purchase_top_category_count', 0), errors='coerce') or 0),
            'second_purchase_top_product': str(row.get('second_purchase_top_product') or ''),
            'second_purchase_top_product_count': int(pd.to_numeric(row.get('second_purchase_top_product_count', 0), errors='coerce') or 0),
            'third_purchase_top_product': str(row.get('third_purchase_top_product') or ''),
            'third_purchase_top_product_count': int(pd.to_numeric(row.get('third_purchase_top_product_count', 0), errors='coerce') or 0),
        }
    except Exception as e:
...
def normalize_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["transaction_id_norm"] = safe_series(out,["transaction_id","transactionid","order_id","order_no","orderno"],"").astype(str).str.strip()
    out["item_id_norm"] = safe_series(out,["item_id","product_code","top_product_code","top_purchased_item_id"],"").astype(str).str.strip()
    out["size_norm"] = safe_series(out,["product_size","size","item_size","top_multi_size_values"],"").astype(str).str.strip()
    out["color_norm"] = safe_series(out,["product_color","color","item_color","top_multi_color_values"],"").astype(str).str.strip()
    ...
    out["revenue_norm"] = to_num(safe_series(out,["total_revenue","revenue","erp_revenue_total","erp_price_total"],0))
    out["ga_revenue_norm"] = to_num(safe_series(out,["ga_purchase_revenue","purchase_revenue","ga_revenue"],0))
    out["metric_revenue_norm"] = out["revenue_norm"].where(out["revenue_norm"] > 0, out["ga_revenue_norm"])
    out["item_revenue_norm"] = to_num(safe_series(out,["item_revenue","purchase_item_revenue","product_revenue","items_revenue","erp_price"],0))
    out["point_used_norm"] = to_num(safe_series(out,["point_used","point_used_total","order_product_use_mileage_total","mileage_used_total"],0))
    ...
    out["signup_30d_no_purchase_flag_norm"] = to_num(safe_series(out,["signup_30d_no_purchase_flag"],0)).astype(int)
    out["first_purchase_within_30d_flag_norm"] = to_num(safe_series(out,["first_purchase_within_30d_flag"],0)).astype(int)
    out["touchable_consent_flag_norm"] = to_num(safe_series(out,["touchable_consent_flag"],0)).astype(int)
    out["first_purchase_category_norm"] = safe_series(out,["first_purchase_category"],"").astype(str)
    out["second_purchase_category_norm"] = safe_series(out,["second_purchase_category"],"").astype(str)
    out["third_purchase_category_norm"] = safe_series(out,["third_purchase_category"],"").astype(str)
    out["fourth_purchase_category_norm"] = safe_series(out,["fourth_purchase_category"],"").astype(str)
    out["first_purchase_product_name_journey_norm"] = safe_series(out,["first_purchase_product_name_journey","first_purchase_product_name"],"").astype(str)
    out["second_purchase_product_name_norm"] = safe_series(out,["second_purchase_product_name"],"").astype(str)
    out["third_purchase_product_name_norm"] = safe_series(out,["third_purchase_product_name"],"").astype(str)
    out["fourth_purchase_product_name_norm"] = safe_series(out,["fourth_purchase_product_name"],"").astype(str)
    out["days_1_to_2_norm"] = to_num(safe_series(out,["days_1_to_2"],0))
    out["days_2_to_3_norm"] = to_num(safe_series(out,["days_2_to_3"],0))
    out["days_3_to_4_norm"] = to_num(safe_series(out,["days_3_to_4"],0))
    out["second_buyer_aov_norm"] = to_num(safe_series(out,["second_buyer_aov"],0))
    out["third_buyer_aov_norm"] = to_num(safe_series(out,["third_buyer_aov"],0))
...
def _clip01(s: pd.Series) -> pd.Series:
    return pd.to_numeric(s, errors="coerce").fillna(0.0).clip(lower=0.0, upper=1.0)
...
def _analysis_features(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    work = df.copy()
    if "last_order_date_norm" in work.columns:
        last_order = pd.to_datetime(work["last_order_date_norm"], errors="coerce")
    else:
        last_order = pd.to_datetime(pd.Series([pd.NaT] * len(work), index=work.index), errors="coerce")
    if "event_date_norm" in work.columns:
        last_event = pd.to_datetime(work["event_date_norm"], errors="coerce")
    else:
        last_event = pd.to_datetime(pd.Series([pd.NaT] * len(work), index=work.index), errors="coerce")
    if "member_reg_datetime_norm" in work.columns:
        signup_dt = pd.to_datetime(work["member_reg_datetime_norm"], errors="coerce")
    else:
        signup_dt = pd.to_datetime(pd.Series([pd.NaT] * len(work), index=work.index), errors="coerce")

    work["recency_days_calc"] = (pd.Timestamp(YESTERDAY_KST) - last_order).dt.days.fillna(365).clip(lower=0)
    work["login_recency_days_calc"] = (pd.Timestamp(YESTERDAY_KST) - last_event).dt.days.fillna(365).clip(lower=0)
    work["tenure_days_calc"] = (pd.Timestamp(YESTERDAY_KST) - signup_dt).dt.days.fillna(0).clip(lower=0)

    numeric_candidates = [
        "sessions_norm", "pageviews_norm", "product_view_count_norm", "add_to_cart_norm",
        "orders_norm", "metric_revenue_norm", "revenue_norm", "item_quantity_norm",
        "item_price_norm", "item_revenue_norm", "point_used_norm", "consent_norm",
        "age_norm", "signup_norm", "purchase_norm", "recency_days_calc", "login_recency_days_calc",
        "tenure_days_calc", "multi_product_customer_flag", "multi_color_flag", "multi_size_flag",
        "repeat_item_flag", "distinct_product_count", "max_distinct_products_in_order",
        "avg_distinct_products_per_order", "multi_product_order_count", "multi_color_product_count",
        "max_color_count", "multi_size_product_count", "max_size_count", "repeat_item_product_count",
        "max_repeat_item_orders"
    ]
    ...
    cat_candidates = ["channel_group_norm", "top_category_norm", "gender_norm", "age_band_norm", "join_device_norm", "paid_detail_norm"]
    for c in cat_candidates:
        if c not in work.columns:
            work[c] = ""
        work[c] = work[c].astype(str).fillna("").replace({"nan": "", "None": ""})

    feat_df = work[numeric_candidates + cat_candidates].copy()
    feat_df = pd.get_dummies(feat_df, columns=cat_candidates, dummy_na=False)
    feat_df = feat_df.replace([np.inf, -np.inf], 0).fillna(0)
    feature_cols = list(feat_df.columns)
    return feat_df, feature_cols

def _fit_driver_model(X: pd.DataFrame, y: pd.Series) -> tuple[pd.Series, pd.Series]:
    if X.empty:
        return pd.Series([0.0] * len(y), index=y.index, dtype=float), pd.Series(dtype=float)
    y = pd.to_numeric(y, errors="coerce").fillna(0).astype(int)
    if y.nunique() < 2 or len(y) < 40 or RandomForestClassifier is None:
        return pd.Series([float(y.mean())] * len(y), index=y.index, dtype=float), pd.Series(0.0, index=X.columns)
    try:
        ...
        return pd.Series(proba, index=y.index, dtype=float), imp
    except Exception:
        return pd.Series([float(y.mean())] * len(y), index=y.index, dtype=float), pd.Series(0.0, index=X.columns)

def _top_feature_name(imp: pd.Series, default: str = "general_signal") -> str:
    if imp is None or imp.empty:
        return default
    top = str(imp.sort_values(ascending=False).index[0])
    top = re.sub(r"^(channel_group_norm_|top_category_norm_|gender_norm_|age_band_norm_|join_device_norm_|paid_detail_norm_)", "", top)
    return top or default

def _driver_action_from_scores(rep: float, first_buy: float, churn: float, ltv: float) -> str:
    if rep >= 0.72 and ltv >= 0.65:
        return "VIP_UPSELL"
    if churn >= 0.68:
    ...
    if rep >= 0.62:
        return "RETENTION_REPURCHASE"
    if ltv >= 0.55:
        return "CATEGORY_CROSSSELL"
    return "GENERAL"

def _priority_tier_from_scores(rep: float, churn: float, ltv: float) -> str:
    mx = max(rep, churn, ltv)
    if mx >= 0.82:
        return "P1"
    if mx >= 0.68:
        return "P2"
...
def _compute_driver_scores_from_frame(user_df: pd.DataFrame) -> pd.DataFrame:
    out = user_df.copy()
    if out.empty:
        for c in ["ml_action_type","ml_priority_tier","next_best_category","dominant_driver","repurchase_30d_score","first_purchase_30d_score","churn_60d_score","ltv_score"]:
            if c not in out.columns:
                out[c] = "" if c in {"ml_action_type","ml_priority_tier","next_best_category","dominant_driver"} else 0.0
        return out

    X, feature_cols = _analysis_features(out)

    orders = pd.to_numeric(out.get("orders_norm", 0), errors="coerce").fillna(0)
    revenue = pd.to_numeric(out.get("metric_revenue_norm", out.get("revenue_norm", 0)), errors="coerce").fillna(0)
    recency = pd.to_numeric(out.get("recency_days_calc", 365), errors="coerce").fillna(365)
    add_to_cart = pd.to_numeric(out.get("add_to_cart_norm", 0), errors="coerce").fillna(0)
    pdp = pd.to_numeric(out.get("product_view_count_norm", 0), errors="coerce").fillna(0)
    tenure = pd.to_numeric(out.get("tenure_days_calc", 0), errors="coerce").fillna(0)

    y_repeat = ((orders >= 2) | ((orders >= 1) & (revenue >= _safe_quantile(revenue[revenue > 0], 0.60, 0.0)))).astype(int)
    y_first = ((orders <= 1) & ((add_to_cart > 0) | (pdp >= 3) | (pd.to_numeric(out.get("sessions_norm", 0), errors="coerce").fillna(0) >= 2))).astype(int)
    ...
    churn_prob, churn_imp = _fit_driver_model(X, y_churn)
    ltv_prob, ltv_imp = _fit_driver_model(X, y_ltv)

    # blend model score with robust business signal to avoid unstable small-sample fits
    rep_signal = (0.40 * _minmax01(orders) + 0.35 * (1.0 - _minmax01(recency)) + 0.25 * _minmax01(revenue)).clip(0, 1)
    first_signal = (0.45 * _minmax01(add_to_cart) + 0.35 * _minmax01(pdp) + 0.20 * (1.0 - _minmax01(orders))).clip(0, 1)
    churn_signal = (0.50 * _minmax01(recency) + 0.20 * (1.0 - _minmax01(pd.to_numeric(out.get("sessions_norm", 0), errors="coerce").fillna(0))) + 0.30 * (1.0 - _minmax01(orders))).clip(0, 1)
    ltv_signal = (0.55 * _minmax01(revenue) + 0.30 * _minmax01(orders) + 0.15 * (1.0 - _minmax01(recency))).clip(0, 1)

    out["repurchase_30d_score"] = _clip01(0.55 * rep_prob + 0.45 * rep_signal)
    out["first_purchase_30d_score"] = _clip01(0.55 * first_prob + 0.45 * first_signal)
    out["churn_60d_score"] = _clip01(0.55 * churn_prob + 0.45 * churn_signal)
    out["ltv_score"] = _clip01(0.55 * ltv_prob + 0.45 * ltv_signal)

    top_repeat = _top_feature_name(rep_imp, "orders")
    top_first = _top_feature_name(first_imp, "add_to_cart")
    top_churn = _top_feature_name(churn_imp, "recency_days")
    top_ltv = _top_feature_name(ltv_imp, "revenue")

    driver_choice = np.vstack([
        out["repurchase_30d_score"].to_numpy(),
        out["first_purchase_30d_score"].to_numpy(),
        out["churn_60d_score"].to_numpy(),
        out["ltv_score"].to_numpy(),
    ]).argmax(axis=0)
    driver_labels = np.array([top_repeat, top_first, top_churn, top_ltv], dtype=object)
    out["dominant_driver"] = driver_labels[driver_choice]
    ...
    out["ml_action_type"] = [
        _driver_action_from_scores(float(r), float(f), float(c), float(l))
        for r, f, c, l in zip(out["repurchase_30d_score"], out["first_purchase_30d_score"], out["churn_60d_score"], out["ltv_score"])
    ]
    out["ml_priority_tier"] = [
    ...
    out.attrs["driver_importance"] = {
        ...
        "first": first_imp.head(12).to_dict(),
        "churn": churn_imp.head(12).to_dict(),
        "ltv": ltv_imp.head(12).to_dict(),
...
def build_deep_driver_insight(df: pd.DataFrame) -> dict:
    ...
    driver_meta = work.attrs.get("driver_importance", {}) if hasattr(work, "attrs") else {}
    summary = {
        "members": int(work["member_id_norm"].replace("", pd.NA).where(work["member_id_norm"].astype(str) != "", work.get("user_id_norm", "")).replace("", pd.NA).nunique()),
    ...
    def _segment_frame(score_col: str, threshold: float, label: str, action: str, driver_key: str) -> dict | None:
        sdf = work[pd.to_numeric(work[score_col], errors="coerce").fillna(0.0) >= threshold].copy()
        if sdf.empty:
            return None
        avg_score = round(float(pd.to_numeric(sdf[score_col], errors="coerce").fillna(0).mean()) * 100.0, 1)
        top_driver = str(driver_meta.get(driver_key, {}))
        if isinstance(driver_meta.get(driver_key), dict) and driver_meta.get(driver_key):
            top_driver = _top_feature_name(pd.Series(driver_meta[driver_key]))
        return {
            "label": label,
            "count": int(sdf["member_id_norm"].replace("", pd.NA).where(sdf["member_id_norm"].astype(str) != "", sdf.get("user_id_norm", "")).replace("", pd.NA).nunique()),
            "avg_score": avg_score,
            "top_channel": top_label(sdf, "channel_group_norm", "미분류"),
            "top_product": top_label(sdf, "purchase_product_name_norm", "미분류"),
            "recommended_action": f"{action} · driver:{top_driver}",
        }

    segments = [x for x in [
        _segment_frame("repurchase_30d_score", 0.65, "Repeat Purchase Drivers", "RETENTION_REPURCHASE", "repeat"),
        _segment_frame("churn_60d_score", 0.65, "Churn Risk Drivers", "CHURN_PREVENTION", "churn"),
        _segment_frame("first_purchase_30d_score", 0.60, "First Buy Drivers", "FIRST_PURCHASE_NUDGE", "first"),
        _segment_frame("ltv_score", 0.70, "High Value Drivers", "VIP_UPSELL", "ltv"),
    ] if x]

    action_dist = distribution(work, "ml_action_type", 10)

    rows = work.copy()
    rows["driver_score"] = rows[["repurchase_30d_score", "first_purchase_30d_score", "churn_60d_score", "ltv_score"]].max(axis=1)
    rows = rows.sort_values(["driver_score", "ltv_score", "repurchase_30d_score"], ascending=[False, False, False])
    cols_map = {
    ...
    return {"summary": summary, "segments": segments, "action_dist": action_dist, "rows": rows_from_df(rows.head(200), cols_map)}
...
def build_purchase_pattern_insight(df: pd.DataFrame) -> dict:
    if df.empty:
        return {
            "summary": {
                "members": 0, "multi_item_buyers": 0, "multi_item_pct": 0.0,
                "multi_color_buyers": 0, "multi_size_buyers": 0, "repeat_item_buyers": 0
            },
            "multi_item_rows": [], "multi_color_rows": [], "multi_size_rows": [], "repeat_item_rows": []
        }

    work = df.copy()
    for c in [
        "member_id_norm", "purchase_product_name_norm", "item_name_norm", "item_id_norm", "product_code_norm",
        "size_norm", "color_norm", "order_id_norm", "transaction_id_norm",
        "multi_product_customer_flag", "multi_color_flag", "multi_size_flag", "repeat_item_flag",
        "distinct_product_count", "max_distinct_products_in_order", "avg_distinct_products_per_order", "multi_product_order_count",
        "multi_color_product_count", "max_color_count", "multi_size_product_count", "max_size_count",
        "repeat_item_product_count", "max_repeat_item_orders",
        "top_multi_color_product_name", "top_multi_size_product_name", "top_repeat_product_name"
    ]:
        if c not in work.columns:
            work[c] = 0 if c.endswith('_flag') or c.endswith('_count') or c.startswith('max_') or c.startswith('avg_') or c.startswith('distinct_') else ""

    # Prefer precomputed member-level features when present
    has_agg = any(c in work.columns for c in ["multi_color_product_count", "multi_size_product_count", "repeat_item_product_count", "distinct_product_count"])
    if has_agg:
        mem = build_user_rows(work)
        total_buyers = int(mem["member_id_norm"].replace("", pd.NA).nunique())
        multi_item = mem[pd.to_numeric(mem.get("multi_product_customer_flag", 0), errors="coerce").fillna(0) >= 1].copy()
        multi_color = mem[pd.to_numeric(mem.get("multi_color_flag", 0), errors="coerce").fillna(0) >= 1].copy()
        multi_size = mem[pd.to_numeric(mem.get("multi_size_flag", 0), errors="coerce").fillna(0) >= 1].copy()
        repeat_item = mem[pd.to_numeric(mem.get("repeat_item_flag", 0), errors="coerce").fillna(0) >= 1].copy()
        summary = {
            "members": total_buyers,
            "multi_item_buyers": int(multi_item["member_id_norm"].nunique()),
            "multi_item_pct": round((int(multi_item["member_id_norm"].nunique()) / total_buyers * 100.0), 1) if total_buyers else 0.0,
            "multi_color_buyers": int(multi_color["member_id_norm"].nunique()),
            "multi_size_buyers": int(multi_size["member_id_norm"].nunique()),
            "repeat_item_buyers": int(repeat_item["member_id_norm"].nunique()),
        }
        multi_item_rows = rows_from_df(
            multi_item.sort_values(["distinct_product_count", "max_distinct_products_in_order", "multi_product_order_count"], ascending=[False, False, False]).head(100),
            {
                "member_id_norm": "member_id", "distinct_product_count": "distinct_product_count",
                "max_distinct_products_in_order": "max_distinct_products_in_order",
                "avg_distinct_products_per_order": "avg_distinct_products_per_order",
                "multi_product_order_count": "multi_product_order_count", "purchase_product_name_norm": "top_product"
            }
        )
        multi_color_rows = rows_from_df(
            multi_color.sort_values(["multi_color_product_count", "max_color_count"], ascending=[False, False]).head(100),
            {
                "member_id_norm": "member_id", "top_multi_color_product_name": "product_name",
                "multi_color_product_count": "multi_color_product_count", "max_color_count": "max_color_count",
                "purchase_product_name_norm": "top_product"
            }
        )
        multi_size_rows = rows_from_df(
            multi_size.sort_values(["multi_size_product_count", "max_size_count"], ascending=[False, False]).head(100),
            {
                "member_id_norm": "member_id", "top_multi_size_product_name": "product_name",
                "multi_size_product_count": "multi_size_product_count", "max_size_count": "max_size_count",
                "purchase_product_name_norm": "top_product"
            }
        )
        repeat_item_rows = rows_from_df(
            repeat_item.sort_values(["repeat_item_product_count", "max_repeat_item_orders"], ascending=[False, False]).head(100),
            {
                "member_id_norm": "member_id", "top_repeat_product_name": "product_name",
                "repeat_item_product_count": "repeat_item_product_count", "max_repeat_item_orders": "max_repeat_item_orders",
                "purchase_product_name_norm": "top_product"
            }
        )
        return {
            "summary": summary,
            "multi_item_rows": multi_item_rows,
            "multi_color_rows": multi_color_rows,
            "multi_size_rows": multi_size_rows,
            "repeat_item_rows": repeat_item_rows,
        }

    # Fallback: row-level inference
    for c in ["member_id_norm", "purchase_product_name_norm", "item_name_norm", "item_id_norm", "product_code_norm", "size_norm", "color_norm", "order_id_norm", "transaction_id_norm"]:
        work[c] = work[c].astype(str).fillna("").str.strip()
    order_key = work["order_id_norm"].where(work["order_id_norm"] != "", work["transaction_id_norm"])
    if (order_key == "").all():
        order_key = work["member_id_norm"] + "|" + work.get("order_date_norm", pd.Series([""] * len(work), index=work.index)).astype(str)
    product_key = work["product_code_norm"].where(work["product_code_norm"] != "", work["item_id_norm"])
    product_name = work["purchase_product_name_norm"].where(work["purchase_product_name_norm"] != "", work["item_name_norm"])
    tmp = work.copy()
    tmp["__order_key"] = order_key.astype(str).str.strip()
    tmp["__product_key"] = product_key.astype(str).str.strip()
    tmp["__product_name"] = product_name.astype(str).str.strip()
    tmp = tmp[(tmp["member_id_norm"] != "") & (tmp["__order_key"] != "")].copy()
    mi = tmp.groupby(["member_id_norm", "__order_key"]).agg(item_count=("__product_key", lambda s: int(pd.Series(s).replace("", pd.NA).dropna().nunique())), product_names=("__product_name", lambda s: ", ".join(pd.Series(s).replace("", pd.NA).dropna().astype(str).unique()[:5]))).reset_index()
    mi = mi[mi["item_count"] >= 2].sort_values(["item_count", "member_id_norm"], ascending=[False, True])
    mc = tmp[(tmp["__product_key"] != "") & (tmp["color_norm"] != "")].groupby(["member_id_norm", "__product_key"]).agg(color_count=("color_norm", lambda s: int(pd.Series(s).replace("", pd.NA).dropna().nunique())), product_name=("__product_name", "first")).reset_index()
    mc = mc[mc["color_count"] >= 2].sort_values(["color_count", "member_id_norm"], ascending=[False, True])
    ms = tmp[(tmp["__product_key"] != "") & (tmp["size_norm"] != "")].groupby(["member_id_norm", "__product_key"]).agg(size_count=("size_norm", lambda s: int(pd.Series(s).replace("", pd.NA).dropna().nunique())), product_name=("__product_name", "first")).reset_index()
    ms = ms[ms["size_count"] >= 2].sort_values(["size_count", "member_id_norm"], ascending=[False, True])
    rp = tmp[tmp["__product_key"] != ""].groupby(["member_id_norm", "__product_key"]).agg(buy_count=("__order_key", lambda s: int(pd.Series(s).replace("", pd.NA).dropna().nunique())), product_name=("__product_name", "first")).reset_index()
    rp = rp[rp["buy_count"] >= 2].sort_values(["buy_count", "member_id_norm"], ascending=[False, True])
    total_buyers = int(tmp["member_id_norm"].replace("", pd.NA).nunique())
    return {
        "summary": {"members": total_buyers, "multi_item_buyers": int(mi["member_id_norm"].nunique()), "multi_item_pct": round((int(mi["member_id_norm"].nunique()) / total_buyers * 100.0), 1) if total_buyers else 0.0, "multi_color_buyers": int(mc["member_id_norm"].nunique()), "multi_size_buyers": int(ms["member_id_norm"].nunique()), "repeat_item_buyers": int(rp["member_id_norm"].nunique())},
        "multi_item_rows": [{"member_id": r["member_id_norm"], "order_key": r["__order_key"], "item_count": int(r["item_count"]), "products": r["product_names"]} for _, r in mi.head(100).iterrows()],
        "multi_color_rows": [{"member_id": r["member_id_norm"], "product_key": r["__product_key"], "product_name": r["product_name"], "color_count": int(r["color_count"])} for _, r in mc.head(100).iterrows()],
        "multi_size_rows": [{"member_id": r["member_id_norm"], "product_key": r["__product_key"], "product_name": r["product_name"], "size_count": int(r["size_count"])} for _, r in ms.head(100).iterrows()],
        "repeat_item_rows": [{"member_id": r["member_id_norm"], "product_key": r["__product_key"], "product_name": r["product_name"], "buy_count": int(r["buy_count"])} for _, r in rp.head(100).iterrows()],
    }
...
def derive_paid_detail_label(df: pd.DataFrame) -> pd.Series:
    ...
    camp = safe_series(df, ["campaign_display_norm","first_campaign","latest_campaign"], "").astype(str).str.lower()
    out = []
    for s,m,c in zip(src, med, camp):
        txt = f"{s} {m} {c}"
        if 'meta' in txt or 'facebook' in txt or 'instagram' in txt:
            out.append('Meta')
        elif 'google' in txt or 'gdn' in txt or 'pmax' in txt or 'demand' in txt:
            out.append('Google')
        elif 'naver' in txt or 'nbs' in txt or 'gfa' in txt:
            out.append('Naver')
        elif 'criteo' in txt:
            out.append('Criteo')
        else:
            out.append('Other Paid')
    return pd.Series(out, index=df.index)

def build_top_related_products(df: pd.DataFrame, top_n: int = 8) -> list[dict[str, Any]]:
    if df.empty or 'purchase_product_name_norm' not in df.columns:
        return []
    work = df.copy()
    work = work[work['purchase_product_name_norm'].astype(str).str.strip() != '']
    if work.empty:
        return []
    buyer_key = work['member_id_norm'].where(work['member_id_norm'].astype(str).str.strip() != '', work['user_id_norm'])
    work['buyer_key_norm'] = buyer_key.astype(str)
    work = work[work['buyer_key_norm'].astype(str).str.strip() != '']
    if work.empty:
        return []
    top_product = work['purchase_product_name_norm'].value_counts().index[0]
    buyers = set(work.loc[work['purchase_product_name_norm'] == top_product, 'buyer_key_norm'].tolist())
    rel = work[work['buyer_key_norm'].isin(buyers) & work['purchase_product_name_norm'].ne(top_product)]
...
def build_purchase_journey_insight(df: pd.DataFrame, start_date: dt.date | None = None, end_date: dt.date | None = None) -> dict:
    ...
    summary = {
        ...
        'first_purchase_top_category': '',
        'first_purchase_top_category_count': 0,
        'second_purchase_top_category': '',
        'second_purchase_top_category_count': 0,
        'third_purchase_top_category': '',
        'third_purchase_top_category_count': 0,
        'second_purchase_top_product': '',
        'second_purchase_top_product_count': 0,
        'third_purchase_top_product': '',
        'third_purchase_top_product_count': 0,
    }
...
def build_signup_30d_insight(df: pd.DataFrame) -> dict:
    if df.empty:
        return {'summary': {}, 'rows': []}
    work = dedupe_user_rows(df.copy())
    for c in ['signup_30d_no_purchase_flag_norm','first_purchase_within_30d_flag_norm','touchable_consent_flag_norm']:
        if c not in work.columns:
            work[c] = 0
        work[c] = pd.to_numeric(work[c], errors='coerce').fillna(0).astype(int)
    focus = work[(work['days_since_signup'].between(0,30, inclusive='both')) | (work['signup_30d_no_purchase_flag_norm'] >= 1)].copy() if 'days_since_signup' in work.columns else work.copy()
    if 'channel_group_norm' in focus.columns and 'channel_group_norm' in focus.columns:
        focus['paid_detail_norm'] = derive_paid_detail_label(focus)
    signups_30d = int(focus['member_id_norm'].replace('', pd.NA).nunique())
    first_purchase_within_30d = int((focus['first_purchase_within_30d_flag_norm'] >= 1).sum())
    signup_30d_no_purchase = int((focus['signup_30d_no_purchase_flag_norm'] >= 1).sum())
    touchable_users = int((focus['touchable_consent_flag_norm'] >= 1).sum())
    summary = {
        'signups_30d': signups_30d,
        'first_purchase_within_30d': first_purchase_within_30d,
        'signup_30d_no_purchase': signup_30d_no_purchase,
        'touchable_users': touchable_users,
        'first_purchase_within_30d_pct': round((first_purchase_within_30d / signups_30d) * 100.0, 1) if signups_30d else 0.0,
        'signup_30d_no_purchase_pct': round((signup_30d_no_purchase / signups_30d) * 100.0, 1) if signups_30d else 0.0,
        'touchable_users_pct': round((touchable_users / signups_30d) * 100.0, 1) if signups_30d else 0.0,
    }
    rows = rows_from_df(focus.sort_values(['signup_30d_no_purchase_flag_norm','touchable_consent_flag_norm','days_since_signup'], ascending=[False,False,True]).head(300), {
        'member_id_norm':'member_id','user_id_norm':'user_id','channel_group_norm':'channel_group','paid_detail_norm':'paid_detail',
        'campaign_display_norm':'campaign','days_since_signup':'days_since_signup','first_purchase_within_30d_flag_norm':'first_purchase_within_30d',
        'signup_30d_no_purchase_flag_norm':'signup_30d_no_purchase','touchable_consent_flag_norm':'touchable','purchase_product_name_norm':'purchase_product_name'
    })
    return {'summary': summary, 'channel_dist': distribution(focus,'channel_group_norm',8), 'paid_detail_dist': distribution(focus,'paid_detail_norm',8), 'rows': rows}
...
def build_bundle(df: pd.DataFrame, start_date: dt.date, end_date: dt.date, period_key: str, period_label: str):
    ...
    member_activity = raw[raw["member_id_norm"] != ""].copy()
    ...
    total_product_raw = buyer_df(raw.copy()) if not raw.empty else raw.copy()
...
<section class="panel active" id="user-view"><div class="section-head"><div><div class="section-title">USER VIEW</div><h2>행동 데이터 · Driver Analysis View</h2></div><div class="download-row">__DOWNLOADS__</div></div><div id="user-sections"></div></section>
...
function renderTotal(section){
  const panel = pickAllPanel(section);
  const s = panel.summary || {};
  return `<section><div class="section-head"><div><div class="section-title">TOTAL OVERVIEW</div><h2>전체 회원 베이스 KPI</h2></div></div>
    <div class="grid-4">
      ...
      <div class="card"><div class="kicker">Revenue</div><div class="kpi">${money(s.revenue||0)}</div></div>
      <div class="card"><div class="kicker">AOV</div><div class="kpi">${money(s.aov||0)}</div></div>
      <div class="card"><div class="kicker">Repeat Buyers</div><div class="kpi">${num(s.repeat_buyers||0)}</div></div>
      <div class="card"><div class="kicker">Avg Orders / Buyer</div><div class="kpi">${Number(s.avg_orders_per_buyer||0).toFixed(2)}</div></div>
      <div class="card"><div class="kicker">Driver Repurchase</div><div class="kpi">${pct((s.avg_repurchase_score||0)*100)}</div></div>
      <div class="card"><div class="kicker">Driver Churn</div><div class="kpi">${pct((s.avg_churn_score||0)*100)}</div></div>
    </div>
    <div class="grid-2">
      <div class="card"><div class="section-title">CHANNEL MIX</div>${bar(panel.channel_dist||[])}</div>
      <div class="card"><div class="section-title">CATEGORY MIX</div>${bar(panel.category_dist||[])}</div>
      <div class="card"><div class="section-title">AGE MIX</div>${bar(panel.age_dist||[])}</div>
      <div class="card"><div class="section-title">GENDER MIX</div>${donut(panel.gender_dist||[])}</div>
    </div>
    <div class="card">${table(panel.rows||[],[['member_id','Member ID'],['user_id','USER_ID'],['channel_group','Channel'],['campaign','Campaign'],['top_category','Top Category'],['purchase_product_name','Top Product'],['orders','Orders'],['revenue','Revenue'],['last_order_date','Last Order'],['ml_repurchase','Repurchase'],['ml_churn','Churn'],['ltv_score','LTV']],['orders','revenue','ml_repurchase','ml_churn','ltv_score'],'total-overview-table')}</div>
  </section>`;
}
function renderTarget(section){
  const cards = section?.cards || [];
  const rows = section?.rows || [];
  const cardHtml = cards.length ? `<div class="grid-4">${cards.map(c=>`<div class="card"><div class="kicker">${esc2(c.label||'Segment')}</div><div class="kpi">${num(c.count||0)}</div><div class="kpi-sub">Top Channel · ${esc2(c.top_channel||'미분류')}</div><div class="kpi-sub">Top Product · ${esc2(c.top_product||'미분류')}</div><div class="kpi-sub">${esc2(c.top_message||'')}</div></div>`).join('')}</div>` : '<div class="card"><div class="kpi-sub">데이터가 없습니다.</div></div>';
  return `<section><div class="section-head"><div><div class="section-title">TARGET</div><h2>Driver Segment Targeting</h2></div></div>${cardHtml}<div class="card">${table(rows,[['member_id','Member ID'],['user_id','USER_ID'],['phone','Phone'],['channel_group','Channel'],['campaign','Campaign'],['product','Product'],['top_category','Top Category'],['dominant_driver','Dominant Driver'],['recommended_action','Action'],['priority_tier','Priority'],['repurchase_score','Repurchase'],['first_buy_score','First Buy'],['churn_score','Churn'],['ltv_score','LTV'],['next_best_category','Next Best Category'],['consent','Consent']],['repurchase_score','first_buy_score','churn_score','ltv_score','consent'], 'target-driver-table')}</div></section>`;
}
function renderMLInsight(section){
  const s = section?.summary || {};
  const segments = section?.segments || [];
  const actions = section?.action_dist || [];
  const rows = section?.rows || [];
  const segHtml = segments.length ? `<div class="grid-4">${segments.map(x=>`<div class="card"><div class="kicker">${esc2(x.label||'Segment')}</div><div class="kpi">${num(x.count||0)}</div><div class="kpi-sub">Avg Score ${pct(x.avg_score||0)}</div><div class="kpi-sub">Top Channel · ${esc2(x.top_channel||'미분류')}</div><div class="kpi-sub">Top Product · ${esc2(x.top_product||'미분류')}</div><div class="kpi-sub">${esc2(x.recommended_action||'')}</div></div>`).join('')}</div>` : '';
  return `<section><div class="section-head"><div><div class="section-title">DRIVER INSIGHT</div><h2>딥 드라이버 분석</h2></div></div>
    <div class="grid-4">
      <div class="card"><div class="kicker">Members</div><div class="kpi">${num(s.members||0)}</div></div>
      <div class="card"><div class="kicker">Avg Repurchase</div><div class="kpi">${pct((s.avg_repurchase_score||0)*100)}</div></div>
      <div class="card"><div class="kicker">Avg Churn</div><div class="kpi">${pct((s.avg_churn_score||0)*100)}</div></div>
      <div class="card"><div class="kicker">High LTV %</div><div class="kpi">${pct(s.high_ltv_pct||0)}</div></div>
    </div>
    ${segHtml}
    <div class="grid-2"><div class="card"><div class="section-title">ACTION DISTRIBUTION</div>${bar(actions)}</div><div class="card"><div class="section-title">TOP ACTION</div><div class="kpi">${esc2(s.top_action||'GENERAL')}</div><div class="kpi-sub">가장 우세한 드라이버 액션</div></div></div>
    <div class="card">${table(rows,[['member_id','Member ID'],['user_id','USER_ID'],['channel_group','Channel'],['campaign','Campaign'],['product','Product'],['dominant_driver','Dominant Driver'],['repurchase_score','Repurchase'],['first_buy_score','First Buy'],['churn_score','Churn'],['ltv_score','LTV'],['recommended_action','Action'],['next_best_category','Next Best Category']],['repurchase_score','first_buy_score','churn_score','ltv_score'],'driver-insight-table')}</div>
  </section>`;
}
function renderPurchasePattern(section){
  const s = section?.summary || {};
  return `<section><div class="section-head"><div><div class="section-title">PURCHASE PATTERN</div><h2>상품/옵션 반복 구매 패턴</h2></div></div>
    <div class="grid-4">
      <div class="card"><div class="kicker">Members</div><div class="kpi">${num(s.members||0)}</div></div>
      <div class="card"><div class="kicker">Multi Item Buyers</div><div class="kpi">${num(s.multi_item_buyers||0)}</div><div class="kpi-sub">${pct(s.multi_item_pct||0)}</div></div>
    ...
    <div class="grid-2">
      <div class="card"><div class="section-title">MULTI ITEM</div>${table(section?.multi_item_rows||[],[['member_id','Member ID'],['top_product','Top Product'],['distinct_products','Distinct Products'],['max_products_in_order','Max / Order'],['avg_products_per_order','Avg / Order'],['orders','Orders']],['distinct_products','max_products_in_order','avg_products_per_order','orders'],'pattern-multi-item')}</div>
      <div class="card"><div class="section-title">MULTI COLOR</div>${table(section?.multi_color_rows||[],[['member_id','Member ID'],['top_product','Top Product'],['color_products','Products'],['max_colors','Max Colors'],['orders','Orders']],['color_products','max_colors','orders'],'pattern-multi-color')}</div>
      <div class="card"><div class="section-title">MULTI SIZE</div>${table(section?.multi_size_rows||[],[['member_id','Member ID'],['top_product','Top Product'],['size_products','Products'],['max_sizes','Max Sizes'],['orders','Orders']],['size_products','max_sizes','orders'],'pattern-multi-size')}</div>
      <div class="card"><div class="section-title">REPEAT ITEM</div>${table(section?.repeat_item_rows||[],[['member_id','Member ID'],['top_product','Top Product'],['repeat_products','Repeat Products'],['max_repeat_orders','Max Repeat Orders'],['orders','Orders']],['repeat_products','max_repeat_orders','orders'],'pattern-repeat-item')}</div>
    </div>
  </section>`;
}
...
function renderPurchaseJourney(section){
  const s=section?.summary||{};
  const catCards = `<div class="grid-4">
    <div class="card"><div class="kicker">1st Purchase Top Category</div><div class="kpi" style="font-size:24px;line-height:1.2;font-family:'Pretendard','Noto Sans KR',sans-serif">${esc2(s.first_purchase_top_category||'-')}</div><div class="kpi-sub">Buyers ${num(s.first_purchase_top_category_count||0)}</div></div>
    <div class="card"><div class="kicker">2nd Purchase Top Category</div><div class="kpi" style="font-size:24px;line-height:1.2;font-family:'Pretendard','Noto Sans KR',sans-serif">${esc2(s.second_purchase_top_category||'-')}</div><div class="kpi-sub">Buyers ${num(s.second_purchase_top_category_count||0)}</div></div>
    <div class="card"><div class="kicker">3rd Purchase Top Category</div><div class="kpi" style="font-size:24px;line-height:1.2;font-family:'Pretendard','Noto Sans KR',sans-serif">${esc2(s.third_purchase_top_category||'-')}</div><div class="kpi-sub">Buyers ${num(s.third_purchase_top_category_count||0)}</div></div>
    <div class="card"><div class="kicker">2nd Purchase Top Product</div><div class="kpi" style="font-size:22px;line-height:1.2;font-family:'Pretendard','Noto Sans KR',sans-serif">${esc2(s.second_purchase_top_product||'-')}</div><div class="kpi-sub">Orders ${num(s.second_purchase_top_product_count||0)}</div></div>
    <div class="card"><div class="kicker">3rd Purchase Top Product</div><div class="kpi" style="font-size:22px;line-height:1.2;font-family:'Pretendard','Noto Sans KR',sans-serif">${esc2(s.third_purchase_top_product||'-')}</div><div class="kpi-sub">Orders ${num(s.third_purchase_top_product_count||0)}</div></div>
  </div>`;
  return `<section><div class="section-head"><div><div class="section-title">PURCHASE JOURNEY</div><h2>첫구매→재구매 전환 흐름</h2></div></div><div class="grid-4"><div class="card"><div class="kicker">Journey Users</div><div class="kpi">${num(s.journey_users||0)}</div></div><div class="card"><div class="kicker">Days 1→2</div><div class="kpi">${Number(s.avg_days_1_to_2||0).toFixed(1)}</div></div><div class="card"><div class="kicker">Days 2→3</div><div class="kpi">${Number(s.avg_days_2_to_3||0).toFixed(1)}</div></div><div class="card"><div class="kicker">Days 3→4</div><div class="kpi">${Number(s.avg_days_3_to_4||0).toFixed(1)}</div></div><div class="card"><div class="kicker">2nd Buyer AOV</div><div class="kpi">${money(s.second_buyer_aov||0)}</div></div><div class="card"><div class="kicker">3rd Buyer AOV</div><div class="kpi">${money(s.third_buyer_aov||0)}</div></div></div>${catCards}<div class="card">${table(section?.rows||[],[['member_id','Member ID'],['user_id','USER_ID'],['channel_group','Channel'],['first_category','1st Category'],['first_product','1st Product'],['second_category','2nd Category'],['second_product','2nd Product'],['third_category','3rd Category'],['third_product','3rd Product'],['days_1_to_2','Days 1→2'],['days_2_to_3','Days 2→3'],['days_3_to_4','Days 3→4']],['days_1_to_2','days_2_to_3','days_3_to_4'],'journey-table')}</div><div class="card">${table(section?.channel_rows||[],[['channel','Channel'],['users','Users'],['avg_days_1_to_2','Avg Days 1→2'],['avg_days_2_to_3','Avg Days 2→3'],['avg_days_3_to_4','Avg Days 3→4']],['users','avg_days_1_to_2','avg_days_2_to_3','avg_days_3_to_4'],'journey-channel-table')}</div></section>`; }
function bindUi(){
...
def load_ml_scores() -> pd.DataFrame:
    ...
    empty = pd.DataFrame(columns=[
        ...
        "ltv_score","next_best_category","dominant_driver",
        "multi_product_customer_flag","multi_color_flag","multi_size_flag","repeat_item_flag",
        "distinct_product_count","max_distinct_products_in_order","avg_distinct_products_per_order","multi_product_order_count",
        "multi_color_product_count","max_color_count","multi_size_product_count","max_size_count","repeat_item_product_count","max_repeat_item_orders",
        "top_multi_color_product_name","top_multi_size_product_name","top_repeat_product_name"
    ])
    if bigquery is None:
    ...
    try:
        ...
        """
        col_df = client.query(cols_sql).to_dataframe()
        available = set(col_df["column_name"].astype(str).tolist()) if not col_df.empty else set()
        def has(c): return c in available
        def pick(*names, cast='STRING', alias=None, default=''):
        ...
        sql = f"""
        SELECT
          {pick('member_id', cast='STRING', alias='member_id_norm')},
          {pick('user_id', cast='STRING', alias='user_id_norm')},
          {pick('ml_action_type', 'crm_action_type', cast='STRING', alias='ml_action_type')},
          {pick('ml_priority_tier', 'priority_tier', cast='STRING', alias='ml_priority_tier')},
          {pick('repurchase_30d_score','repurchase_score', cast='FLOAT64', alias='repurchase_30d_score', default='0')},
          {pick('first_purchase_30d_score','first_purchase_score', cast='FLOAT64', alias='first_purchase_30d_score', default='0')},
          {pick('churn_60d_score','churn_risk_score', cast='FLOAT64', alias='churn_60d_score', default='0')},
          {pick('ltv_score', cast='FLOAT64', alias='ltv_score', default='0')},
          {pick('next_best_category', cast='STRING', alias='next_best_category')},
          CAST('' AS STRING) AS dominant_driver,
          {pick('multi_product_customer_flag', cast='FLOAT64', alias='multi_product_customer_flag', default='0')},
          {pick('multi_color_flag', cast='FLOAT64', alias='multi_color_flag', default='0')},
          {pick('multi_size_flag', cast='FLOAT64', alias='multi_size_flag', default='0')},
          {pick('repeat_item_flag', cast='FLOAT64', alias='repeat_item_flag', default='0')},
          {pick('distinct_product_count', cast='FLOAT64', alias='distinct_product_count', default='0')},
          {pick('max_distinct_products_in_order', cast='FLOAT64', alias='max_distinct_products_in_order', default='0')},
          {pick('avg_distinct_products_per_order', cast='FLOAT64', alias='avg_distinct_products_per_order', default='0')},
          {pick('multi_product_order_count', cast='FLOAT64', alias='multi_product_order_count', default='0')},
          {pick('multi_color_product_count', cast='FLOAT64', alias='multi_color_product_count', default='0')},
          {pick('max_color_count', cast='FLOAT64', alias='max_color_count', default='0')},
          {pick('multi_size_product_count', cast='FLOAT64', alias='multi_size_product_count', default='0')},
          {pick('max_size_count', cast='FLOAT64', alias='max_size_count', default='0')},
          {pick('repeat_item_product_count', cast='FLOAT64', alias='repeat_item_product_count', default='0')},
          {pick('max_repeat_item_orders', cast='FLOAT64', alias='max_repeat_item_orders', default='0')},
          {pick('top_multi_color_product_name', cast='STRING', alias='top_multi_color_product_name')},
          {pick('top_multi_size_product_name', cast='STRING', alias='top_multi_size_product_name')},
          {pick('top_repeat_product_name', cast='STRING', alias='top_repeat_product_name')}
        FROM `{table_name}`
        """
        ...
        for c in empty.columns:
            if c not in df.columns:
                df[c] = '' if c in {'member_id_norm','user_id_norm','ml_action_type','ml_priority_tier','next_best_category','dominant_driver','top_multi_color_product_name','top_multi_size_product_name','top_repeat_product_name'} else 0.0
        for c in [x for x in df.columns if x not in ['member_id_norm','user_id_norm','ml_action_type','ml_priority_tier','next_best_category','dominant_driver','top_multi_color_product_name','top_multi_size_product_name','top_repeat_product_name']]:
            df[c] = pd.to_numeric(df[c], errors='coerce').fillna(0.0)
        for c in ['member_id_norm','user_id_norm','ml_action_type','ml_priority_tier','next_best_category','dominant_driver','top_multi_color_product_name','top_multi_size_product_name','top_repeat_product_name']:
            df[c] = df[c].astype(str).fillna('').str.strip()
        _ML_SCORES_CACHE = df
        return df.copy()
    except Exception as e:
        print(f"[WARN] load_ml_scores fallback to in-app deep analysis: {type(e).__name__}: {e}")
        _ML_SCORES_CACHE = empty
        return empty.copy()

def merge_ml_scores(user_df: pd.DataFrame, ml_scores: pd.DataFrame) -> pd.DataFrame:
    out = user_df.copy()
    string_cols = ["ml_action_type","ml_priority_tier","next_best_category","dominant_driver"]
    score_cols = ["repurchase_30d_score","first_purchase_30d_score","churn_60d_score","ltv_score"]
    extra_numeric_cols = [
        "multi_product_customer_flag","multi_color_flag","multi_size_flag","repeat_item_flag","distinct_product_count",
        "max_distinct_products_in_order","avg_distinct_products_per_order","multi_product_order_count","multi_color_product_count",
        "max_color_count","multi_size_product_count","max_size_count","repeat_item_product_count","max_repeat_item_orders"
    ]
    extra_string_cols = ["top_multi_color_product_name","top_multi_size_product_name","top_repeat_product_name"]
    for c in string_cols + extra_string_cols:
        if c not in out.columns:
            out[c] = ""
    for c in score_cols + extra_numeric_cols:
        if c not in out.columns:
            out[c] = 0.0
    ...
    if ml_scores is not None and not ml_scores.empty:
        ml = ml_scores.copy()
        ml["member_id_norm"] = ml.get("member_id_norm", "").astype(str).fillna("").str.strip()
        ml["user_id_norm"] = ml.get("user_id_norm", "").astype(str).fillna("").str.strip()
        value_cols = string_cols + score_cols + extra_numeric_cols + extra_string_cols
        m1 = merged.merge(ml[["member_id_norm"] + value_cols].rename(columns={c: f"{c}__m1" for c in value_cols}), on="member_id_norm", how="left")
        m2 = merged.merge(ml[["user_id_norm"] + value_cols].rename(columns={c: f"{c}__m2" for c in value_cols}), on="user_id_norm", how="left")
        for c in value_cols:
            if c in string_cols + extra_string_cols:
                base = merged[c].astype(str).fillna("")
                v1 = m1.get(f"{c}__m1", "").astype(str).fillna("") if f"{c}__m1" in m1.columns else ""
                v2 = m2.get(f"{c}__m2", "").astype(str).fillna("") if f"{c}__m2" in m2.columns else ""
                merged[c] = np.where(base != "", base, np.where(v1 != "", v1, np.where(v2 != "", v2, "")))
            else:
                base = pd.to_numeric(merged[c], errors="coerce").fillna(0.0)
                v1 = pd.to_numeric(m1.get(f"{c}__m1", 0.0), errors="coerce").fillna(0.0)
                v2 = pd.to_numeric(m2.get(f"{c}__m2", 0.0), errors="coerce").fillna(0.0)
                merged[c] = np.where(base > 0, base, np.where(v1 > 0, v1, np.where(v2 > 0, v2, 0.0)))

    # Deep in-app driver analysis fallback / complement
    needs_driver = (
        (pd.to_numeric(merged.get("repurchase_30d_score", 0), errors="coerce").fillna(0) <= 0).all()
        or (merged.get("ml_action_type", pd.Series([""] * len(merged), index=merged.index)).astype(str).str.strip() == "").all()
    )
    if needs_driver:
        merged = _compute_driver_scores_from_frame(merged)
    else:
        # still refresh dominant driver for richer analysis
        enriched = _compute_driver_scores_from_frame(merged.copy())
        if "dominant_driver" in enriched.columns:
            merged["dominant_driver"] = enriched["dominant_driver"]
