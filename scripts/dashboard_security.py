...
SENSITIVE_KEYS = {
    ...
    "tel", "telephone", "contact", "customer_name", "member_name", "name", "address",
    "customer_id", "member_id", "user_id", "ci", "di", "rrn", "resident_number",
}
...
def mask_text(text: str) -> tuple[str, int]:
    ...
    text = RRN_RE.sub(repl, text)
    return text, count


def sanitize_obj(obj: Any, key: str | None = None) -> tuple[Any, int]:
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            ...
            if lower in SENSITIVE_KEYS and v not in (None, ""):
                ...
                out[k], n = sanitize_obj(v, lower)
                count += n
        return out, count
    if isinstance(obj, list):
        result, count = [], 0
        for v in obj:
            clean, n = sanitize_obj(v, key)
            result.append(clean)
            count += n
        return result, count
    ...
    for root in ROOTS:
        ...
        for path in root.rglob("*"):
            ...
            if path.stat().st_size > 25_000_000:
                continue
            scanned += 1
            if args.sanitize and path.suffix.lower() == ".json":
                masked += sanitize_json(path)
            elif args.sanitize and path.suffix.lower() == ".csv":
                masked += sanitize_csv(path)
            try:
                text = path.read_text(encoding="utf-8", errors="ignore")
            except Exception:
                continue
            hits = scan_secrets(path, text)
            if hits:
                secret_hits.append(f"{path}: {', '.join(hits)}")
            if path.suffix.lower() in {".html", ".js", ".txt"}:
                pii = len(EMAIL_RE.findall(text)) + len(PHONE_RE.findall(text)) + len(RRN_RE.findall(text))
                if pii:
                    secret_hits.append(f"{path}: unmasked PII-like strings={pii}")
    print(f"[SECURITY] scanned={scanned} masked={masked}")
    if secret_hits:
        print("[SECURITY] blocked findings:")
        for hit in secret_hits[:100]:
            print(f" - {hit}")
        return 1
    return 0
