...
AD_CACHE = OUT / "performance_ad_spend_daily.csv"
...
def load_ad_rows():
    if not AD_CACHE.exists() or AD_CACHE.stat().st_size == 0:
        return []
...
def build_initial_payload(payload, initial_start, max_date):
    ...
    return {
        ...
        "top": top,
        "ads": [r for r in payload.get("ads", []) if initial_start <= r["date"] <= max_date],
    }
...
.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}.pager{display:flex;justify-content:center;gap:7px;margin:18px 0 8px}.pager button{min-width:38px}.pager button.active{background:var(--accent);border-color:var(--accent);color:white}.paid-title{font-size:18px;margin:24px 0 10px}.paid-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:22px}.paid-card{background:#0b1321;border:1px solid var(--line);border-radius:13px;padding:14px}.paid-card h3{font-size:14px;margin:0 0 10px}.paid-row{display:flex;justify-content:space-between;color:var(--muted);font-size:11px;margin-top:6px}.paid-row b{color:var(--text)}.head{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:8px}.head h2{font-size:16px;margin:2px 0 0;overflow-wrap:anywhere}.rank{color:var(--muted);font-size:11px}.mini{color:var(--muted);font-size:12px;text-align:right}
...
<div class="cards">
  ...
  <div class="card"><div class="k">Ad Spend (VAT 제외)</div><div class="v" id="totalSpend">-</div><div class="yoy">선택 기간 집행액</div></div>
  <div class="card"><div class="k">GA ROAS</div><div class="v" id="gaRoas">-</div><div class="yoy">리포트 GA 매출 ÷ 광고비</div></div>
  <div class="card"><div class="k">Media ROAS</div><div class="v" id="mediaRoas">-</div><div class="yoy">매체 전환매출 ÷ 광고비</div></div>
</div>
<h2 class="paid-title">Paid Media</h2>
<div id="paidGrid" class="paid-grid"><div class="paid-card empty">광고비 데이터 불러오는 중...</div></div>
<h2 class="paid-title">Source / Medium Top 20</h2>
...
function aggregateAds(rows){return rows.reduce((a,r)=>{a.spend+=Number(r.spend)||0;a.platform_revenue+=Number(r.platform_revenue)||0;a.report_ga_revenue+=Number(r.report_ga_revenue)||0;return a},{spend:0,platform_revenue:0,report_ga_revenue:0})}
function roasText(revenue,spend){return spend?((revenue/spend)*100).toFixed(0)+'%':'-'}
function updatePaid(ads){
  const total=aggregateAds(ads);$('totalSpend').textContent=fmtK(total.spend);$('gaRoas').textContent=roasText(total.report_ga_revenue,total.spend);$('mediaRoas').textContent=roasText(total.platform_revenue,total.spend);
  const groups=new Map();for(const r of ads){let a=groups.get(r.channel);if(!a){a={spend:0,platform_revenue:0,report_ga_revenue:0};groups.set(r.channel,a)}a.spend+=Number(r.spend)||0;a.platform_revenue+=Number(r.platform_revenue)||0;a.report_ga_revenue+=Number(r.report_ga_revenue)||0}
  const el=$('paidGrid'),items=[...groups.entries()].filter(x=>x[1].spend>0).sort((a,b)=>b[1].spend-a[1].spend);el.innerHTML=items.length?'':'<div class="paid-card empty">선택 기간 광고비 데이터가 없습니다.</div>';
  for(const [name,a] of items){const d=document.createElement('div');d.className='paid-card';d.innerHTML=`<h3>${esc(name)}</h3><div class="paid-row"><span>Ad Spend</span><b>${fmtK(a.spend)}</b></div><div class="paid-row"><span>GA Revenue</span><b>${fmtK(a.report_ga_revenue)}</b></div><div class="paid-row"><span>GA ROAS</span><b>${roasText(a.report_ga_revenue,a.spend)}</b></div><div class="paid-row"><span>Media ROAS</span><b>${roasText(a.platform_revenue,a.spend)}</b></div>`;el.appendChild(d)}
}
