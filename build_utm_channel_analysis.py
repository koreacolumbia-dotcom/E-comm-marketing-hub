...
def make_legend_html() -> str:
    items = [
        ("전체", "all"), ("Google", "google"), ("Meta", "meta"), ("Naver", "naver"), ("Kakao", "kakao"),
        ("EDM", "edm"), ("LMS", "lms"), ("Organic", "organic"), ("Paid", "paid"), ("Owned", "owned"), ("SNS", "sns")
    ]
    chips = []
    for label, key in items:
        style = tone_style(key if key != "all" else "other")
        active = " active" if key == "all" else ""
        chips.append(f'<button type="button" class="legend-chip media-filter-chip{active}" data-media-filter="{html_escape(key)}" style="{style}">{html_escape(label)}</button>')
    return "".join(chips)
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* v6 · alert card width + media chip filtering + source/medium KPI panel */
  #alertGrid.alerts-grid{display:block!important;width:100%}
  #alertGrid .alerts-wrap{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px;width:100%}
  #alertGrid .alert-lane{min-width:0;width:100%}
  #alertGrid .alert-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:12px;align-items:stretch}
  #alertGrid .alert-list .alert-card{width:100%;min-width:0}
  #alertGrid .alert-desc{word-break:break-word;overflow-wrap:anywhere}
  #alertGrid .badge-wrap{max-width:46%;overflow:hidden}.badge-wrap .badge{max-width:100%;overflow:hidden;text-overflow:ellipsis}
  .legend-row{justify-content:flex-end}
  .media-filter-chip{cursor:pointer;user-select:none}
  .media-filter-chip.active{transform:translateY(-1px);outline:2px solid var(--tone-dot);outline-offset:2px;box-shadow:0 12px 26px rgba(15,23,42,.10),inset 0 1px 0 rgba(255,255,255,.75)}
  .media-filter-chip:not(.active){opacity:.88}
  .media-filter-chip:hover{opacity:1;transform:translateY(-1px)}
  .source-toolbar{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:10px}
  .source-selected-pill{height:30px;padding:0 11px;border-radius:999px;background:#0f172a;color:#fff;font-size:11px;font-weight:950;display:inline-flex;align-items:center;gap:6px}
  .source-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}
  .source-card{min-width:0}
  .source-main{grid-template-columns:repeat(3,minmax(0,1fr))}
  .source-main .metric-value{font-size:16px;word-break:break-word}
  .source-mini{grid-template-columns:repeat(4,minmax(0,1fr))}
  .source-mini .mini-value{overflow:hidden;text-overflow:ellipsis}
  .media-card{cursor:pointer}
  .media-card:hover .media-brand{color:var(--tone-fg)}
  @media (max-width:1360px){#alertGrid .alerts-wrap{grid-template-columns:1fr}.source-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
  @media (max-width:760px){#alertGrid .alert-list{grid-template-columns:1fr}.source-grid,.source-main{grid-template-columns:1fr}.source-mini{grid-template-columns:repeat(2,minmax(0,1fr))}}
  ...
  /* Source/Medium KPI detail patch · 2026-05-18 */
  .legend-chip{cursor:pointer;font-family:inherit}
  .legend-chip.active{outline:2px solid var(--tone-dot);outline-offset:2px;box-shadow:0 10px 24px rgba(15,23,42,.08), inset 0 1px 0 rgba(255,255,255,.75)}
  .source-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:14px}
  .source-card{position:relative;overflow:hidden;border:1px solid var(--tone-border);border-radius:22px;background:linear-gradient(180deg,#fff 0%,var(--tone-bg) 100%);box-shadow:var(--shadow-soft);padding:15px;transition:all .22s var(--motion-ease)}
  .source-card:hover{transform:translateY(-3px);box-shadow:var(--shadow-hover)}
  .source-head{display:flex;justify-content:space-between;align-items:flex-start;gap:10px;margin-bottom:12px}
  .source-title{font-size:14px;font-weight:950;color:#0f172a;line-height:1.25;word-break:break-word}
  .source-sub{margin-top:4px;font-size:11px;font-weight:850;color:#94a3b8;line-height:1.35;word-break:break-word}
  .source-main{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:12px 0}
  .source-main .metric-box{border-radius:15px;background:rgba(255,255,255,.62);border:1px solid rgba(255,255,255,.72);padding:10px}
  .metric-label{font-size:10px;font-weight:950;color:#94a3b8;text-transform:uppercase;letter-spacing:.04em}
  .metric-value{margin-top:5px;font-size:17px;font-weight:950;color:#0f172a;letter-spacing:-.03em}
  .source-mini{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px;margin-top:10px}
  .source-mini .mini-box{border-radius:13px;background:rgba(255,255,255,.50);padding:8px;min-width:0}
  .source-mini .mini-label{font-size:10px;font-weight:900;color:#94a3b8;white-space:nowrap}
  .source-mini .mini-value{margin-top:4px;font-size:13px;font-weight:950;color:#334155;white-space:nowrap}
  .source-empty{grid-column:1/-1}

  @media (max-width:1360px){.main{padding:24px 18px 34px}.kpi-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.alerts-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.media-grid,.source-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.chart-grid{grid-template-columns:1fr}}
  @media (max-width:900px){.alerts-wrap{grid-template-columns:1fr}}
  @media (max-width:760px){.main{padding:18px 12px 26px}.page-title{font-size:32px}.topbar{flex-direction:column}.kpi-grid,.alerts-grid,.media-grid,.source-grid{grid-template-columns:1fr}.panel{border-radius:22px}}
  ...
  @media (max-width:1360px){.kpi-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.alerts-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.media-grid,.source-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
  @media (max-width:900px){.alerts-wrap{grid-template-columns:1fr}}
  @media (max-width:760px){.main{padding:18px 12px 28px}.topbar{align-items:flex-start}.page-title{font-size:26px}.kpi-grid,.alerts-grid,.media-grid,.source-grid{grid-template-columns:1fr}.panel{border-radius:20px}.filter-row,.table-tools,.section-head{align-items:flex-start}.right-controls,.left-controls{width:100%}.control[type="search"]{width:100%!important}}
...
<body>
  <div class="layout">
    <main class="main">
      ...
      <section class="panel card-section"><div class="section-head"><div><h2 class="section-title">매체별 한눈에 보기</h2><div class="chart-sub">오른쪽 칩을 클릭하면 해당 매체만 필터링됩니다.</div></div><div class="legend-row" id="mediaFilterChips">__LEGEND_HTML__</div></div><div class="media-grid" id="mediaGrid"></div><div class="section-head" style="margin-top:18px"><div><h3 class="chart-title">소스/매체별 핵심 지표</h3><div class="chart-sub">선택한 매체 기준으로 PV, 구매 CVR, 회원가입 CVR, 1인당 PV를 따로 확인합니다.</div><div class="source-toolbar"><span class="source-selected-pill" id="sourceSelectedLabel">전체 매체</span></div></div></div><div class="source-grid" id="sourceMediumGrid"></div></section>
...
<script>
const rows = __ROWS__; const tonePalette = __TONE_PALETTE__; const allMinDate = "__MIN_DATE__"; const allMaxDate = "__MAX_DATE__"; let barMetric = "revenue"; let revenueChart = null; let lineChart = null; let selectedMedia = "all";
...
function mediaFilterMatch(r){if(!selectedMedia || selectedMedia==='all') return true; return toneKey(preferredMediaName(r))===selectedMedia;}
...
function renderMediaGrid(arr){
  ...
  mount.innerHTML = grouped.slice(0,8).map(r=>{
    ...
    return `<div class="media-card" data-media-card="${toneKey(name)}" title="${esc(name)}만 보기" style="--tone-bg:${t.bg};--tone-fg:${t.fg};--tone-border:${t.border};--tone-dot:${t.dot}">
      ...
      <div class="media-stats"><div><div class="media-stat-label">세션</div><div class="media-stat-value">${num(r.sessions)}</div></div><div><div class="media-stat-label">구매 전환율</div><div class="media-stat-value">${pct(r.buy_cvr)}</div></div><div><div class="media-stat-label">회원가입 CVR</div><div class="media-stat-value">${pct(r.signup_cvr)}</div></div><div><div class="media-stat-label">1인당 PV</div><div class="media-stat-value">${one(r.pv_per_user)}</div></div></div>
      <div class="spark-wrap">${makeSparkSvg((trendMap[name]||[]).slice(-14).map(x=>n(x.revenue)), t.dot)}</div>
    </div>`;
  }).join('');
}
function selectedMediaLabel(){
  const active=document.querySelector(`[data-media-filter="${selectedMedia}"]`);
  return active ? active.textContent.trim() : '전체';
}
function renderSourceMediumGrid(arr){
  const mount=document.getElementById('sourceMediumGrid');
  const label=document.getElementById('sourceSelectedLabel');
  if(label) label.textContent = selectedMedia==='all' ? '전체 매체' : `${selectedMediaLabel()} 선택 중`;
  if(!mount) return;
  const grouped=groupBy(arr.map(r=>({...r, media_name:preferredMediaName(r)})), ['media_name','source','medium']).sort((a,b)=>n(b.revenue)-n(a.revenue)||n(b.sessions)-n(a.sessions));
  if(!grouped.length){mount.innerHTML='<div class="empty source-empty">조건에 맞는 소스/매체 데이터가 없습니다.</div>'; return;}
  mount.innerHTML=grouped.slice(0,24).map(r=>{
    const name=r.media_name; const t=tone(name); const sourceMedium=`${r.source||'-'} / ${r.medium||'-'}`;
    const estimatedPv=n(r.pv_per_user)*n(r.users);
    return `<div class="source-card" style="--tone-bg:${t.bg};--tone-fg:${t.fg};--tone-border:${t.border};--tone-dot:${t.dot}">
      <div class="source-head"><div><div class="source-title">${esc(sourceMedium)}</div><div class="source-sub">${badge(name,name)}</div></div></div>
      <div class="source-main">
        <div class="metric-box"><div class="metric-label">매출</div><div class="metric-value">${money(r.revenue)}</div></div>
        <div class="metric-box"><div class="metric-label">세션</div><div class="metric-value">${num(r.sessions)}</div></div>
        <div class="metric-box"><div class="metric-label">PV</div><div class="metric-value">${num(estimatedPv)}</div></div>
      </div>
      <div class="source-mini">
        <div class="mini-box"><div class="mini-label">구매 CVR</div><div class="mini-value">${pct(r.buy_cvr)}</div></div>
        <div class="mini-box"><div class="mini-label">가입 CVR</div><div class="mini-value">${pct(r.signup_cvr)}</div></div>
        <div class="mini-box"><div class="mini-label">PV/USER</div><div class="mini-value">${one(r.pv_per_user)}</div></div>
        <div class="mini-box"><div class="mini-label">가입자 PV</div><div class="mini-value">${one(r.avg_signup_user_pv)}</div></div>
      </div>
      <div class="source-mini">
        <div class="mini-box"><div class="mini-label">사용자</div><div class="mini-value">${num(r.users)}</div></div>
        <div class="mini-box"><div class="mini-label">회원가입</div><div class="mini-value">${num(r.signups)}</div></div>
        <div class="mini-box"><div class="mini-label">구매자</div><div class="mini-value">${num(r.buyers)}</div></div>
        <div class="mini-box"><div class="mini-label">AOV</div><div class="mini-value">${money(r.aov_per_buyer)}</div></div>
      </div>
      <div class="source-mini">
        <div class="mini-box"><div class="mini-label">구매건수</div><div class="mini-value">${num(r.purchase)}</div></div>
        <div class="mini-box"><div class="mini-label">가입→구매</div><div class="mini-value">${num(r.signup_to_buyers)}</div></div>
        <div class="mini-box"><div class="mini-label">매출/세션</div><div class="mini-value">${money(n(r.sessions)?n(r.revenue)/n(r.sessions):0)}</div></div>
        <div class="mini-box"><div class="mini-label">매출/USER</div><div class="mini-value">${money(n(r.users)?n(r.revenue)/n(r.users):0)}</div></div>
      </div>
    </div>`;
  }).join('');
}
function buildPeriodRows(arr){return groupBy(arr.map(r=>({...r, media_name:preferredMediaName(r)})), ['media_name','source','medium','campaign']).sort((a,b)=>n(b.revenue)-n(a.revenue)||n(b.sessions)-n(a.sessions));} function buildPeriodTable(grouped){return `<table><thead><tr><th class="rank">#</th><th>채널 + 매체</th><th>소스 / 매체</th><th>캠페인</th><th class="num">세션</th><th class="num">매출</th><th class="num">구매 전환율</th><th class="num">AOV</th><th class="num">구매수</th><th class="num">회원가입 CVR</th><th class="num">사용자</th><th class="num">회원가입</th><th class="num">가입자 평균 PV</th><th class="num">PV/사용자</th></tr></thead><tbody>${grouped.length?grouped.map((r,i)=>`<tr><td class="rank">${i+1}</td><td>${badge(r.media_name,r.media_name)}</td><td>${esc(r.source)} / ${esc(r.medium)}</td><td class="wide">${esc(r.campaign)}</td><td class="num">${num(r.sessions)}</td><td class="num strong">${money(r.revenue)}</td><td class="num">${pct(r.buy_cvr)}</td><td class="num">${money(r.aov_per_buyer)}</td><td class="num">${num(r.buyers)}</td><td class="num">${pct(r.signup_cvr)}</td><td class="num">${num(r.users)}</td><td class="num">${num(r.signups)}</td><td class="num">${one(r.avg_signup_user_pv)}</td><td class="num">${one(r.pv_per_user)}</td></tr>`).join(''):`<tr><td colspan="14" class="empty">조건에 맞는 데이터가 없습니다.</td></tr>`}</tbody></table>`;} function buildDailyRows(arr){return groupBy(arr,['event_dt','channel_group']).sort((a,b)=>String(b.event_dt).localeCompare(String(a.event_dt))||n(b.revenue)-n(a.revenue));} function buildDailyTable(grouped){return `<table><thead><tr><th>일자</th><th>채널</th><th class="num">세션</th><th class="num">사용자</th><th class="num">회원가입</th><th class="num">회원가입 CVR</th><th class="num">구매수</th><th class="num">구매 전환율</th><th class="num">매출</th><th class="num">AOV</th></tr></thead><tbody>${grouped.length?grouped.map(r=>`<tr><td>${esc(r.event_dt)}</td><td>${badge(r.channel_group,r.channel_group)}</td><td class="num">${num(r.sessions)}</td><td class="num">${num(r.users)}</td><td class="num">${num(r.signups)}</td><td class="num">${pct(r.signup_cvr)}</td><td class="num">${num(r.buyers)}</td><td class="num">${pct(r.buy_cvr)}</td><td class="num strong">${money(r.revenue)}</td><td class="num">${money(r.aov_per_buyer)}</td></tr>`).join(''):`<tr><td colspan="10" class="empty">조건에 맞는 데이터가 없습니다.</td></tr>`}</tbody></table>`;} function buildDetailRows(arr){return [...arr].sort((a,b)=>n(b.revenue)-n(a.revenue)||n(b.sessions)-n(a.sessions));} function buildDetailTable(sorted){return `<table><thead><tr><th class="rank">#</th><th>채널 + 매체</th><th>소스 / 매체</th><th>캠페인</th><th class="num">세션</th><th class="num">매출</th><th class="num">구매 전환율</th><th class="num">AOV</th><th class="num">구매수</th><th class="num">회원가입 CVR</th><th class="num">사용자</th><th class="num">회원가입</th><th class="num">구매건수</th><th class="num">가입자 평균 PV</th><th class="num">PV/사용자</th></tr></thead><tbody>${sorted.length?sorted.map((r,i)=>`<tr><td class="rank">${i+1}</td><td>${badge(preferredMediaName(r),preferredMediaName(r))}</td><td>${esc(r.source)} / ${esc(r.medium)}</td><td class="wide">${esc(r.campaign)}</td><td class="num">${num(r.sessions)}</td><td class="num strong">${money(r.revenue)}</td><td class="num">${pct(r.buy_cvr)}</td><td class="num">${money(r.aov_per_buyer)}</td><td class="num">${num(r.buyers)}</td><td class="num">${pct(r.signup_cvr)}</td><td class="num">${num(r.users)}</td><td class="num">${num(r.signups)}</td><td class="num">${num(r.purchase)}</td><td class="num">${one(r.avg_signup_user_pv)}</td><td class="num">${one(r.pv_per_user)}</td></tr>`).join(''):`<tr><td colspan="15" class="empty">조건에 맞는 데이터가 없습니다.</td></tr>`}</tbody></table>`;}
