...
<body>
  <div class="layout">
    <main class="main">
      ...
      <section class="panel card-section"><div class="section-head"><div><h2 class="section-title">매체별 한눈에 보기</h2><div class="chart-sub">오른쪽 칩을 클릭하면 해당 매체만 필터링됩니다.</div></div><div class="legend-row" id="mediaFilterChips">__LEGEND_HTML__</div></div><div class="media-grid" id="mediaGrid"></div><div class="section-head" style="margin-top:18px"><div><h3 class="chart-title">소스/매체별 핵심 지표</h3><div class="chart-sub">PV, CVR, 회원가입 CVR, 1인당 PV까지 함께 확인</div></div></div><div class="source-grid" id="sourceMediumGrid"></div></section>
...
function renderSourceMediumGrid(arr){
  ...
  mount.innerHTML=grouped.slice(0,12).map(r=>{
    ...
    return `<div class="source-card" style="--tone-bg:${t.bg};--tone-fg:${t.fg};--tone-border:${t.border};--tone-dot:${t.dot}">
      ...
      <div class="source-main"><div class="metric-box"><div class="metric-label">매출</div><div class="metric-value">${money(r.revenue)}</div></div><div class="metric-box"><div class="metric-label">세션</div><div class="metric-value">${num(r.sessions)}</div></div></div>
      <div class="source-mini">
        <div class="mini-box"><div class="mini-label">PV/USER</div><div class="mini-value">${one(r.pv_per_user)}</div></div>
        <div class="mini-box"><div class="mini-label">BUY CVR</div><div class="mini-value">${pct(r.buy_cvr)}</div></div>
      ...
      <div class="source-mini">
        ...
        <div class="mini-box"><div class="mini-label">가입</div><div class="mini-value">${num(r.signups)}</div></div>
