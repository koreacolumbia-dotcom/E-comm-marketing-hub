...
<div class="control product-input"><label>상품코드 복수 검색</label><input id="productCodeFilter" placeholder="C72YL3596774, C75BK9168010" /></div>
...
function splitTokens(v){return String(v||'').split(/[\\s,;|]+/).map(x=>x.trim()).filter(Boolean);} function codes(){return splitTokens(document.getElementById('productCodeFilter').value).map(x=>x.toUpperCase());} function q(){return(document.getElementById('keywordFilterTop').value||'').trim().toLowerCase();}
...
function groupWeekly(rows){const map=new Map();rows.forEach(r=>{const d=new Date(r.date+'T00:00:00');const day=d.getDay();const monday=new Date(d);monday.setDate(d.getDate()-((day+6)%7));const key=monday.toISOString().slice(0,10);const p=r.period||'TY';const mk=p+'|'+key;if(!map.has(mk))map.set(mk,{period:p,date:key,searches:0,orders:0,purchase_qty:0,erp_revenue:0});const x=map.get(mk);x.searches+=Number(r.searches||0);x.orders+=Number(r.orders||0);x.purchase_qty+=Number(r.purchase_qty||0);x.erp_revenue+=Number(r.erp_revenue||0)});return Array.from(map.values()).sort((a,b)=>a.date.localeCompare(b.date)||a.period.localeCompare(b.period))}
