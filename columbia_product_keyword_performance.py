...
function scopedProductRows(){
  ...
  const ty=latestPeriodRows(base,'TY');
  const ly=latestPeriodRows(base,'LY');
  return [...ty,...ly];
}
...
function renderTable(){
  ...
  if(!rows.length){
    tbody.innerHTML='<tr><td colspan="11" style="text-align:center;color:#64748b;padding:28px;font-weight:900;">선택한 DAILY/WEEK 기준에 표시할 상품코드 상세 데이터가 없습니다.</td></tr>';
    return;
  }
